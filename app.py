...
def check_login():
    if not st.session_state.get("logged_in"):
        ...
        with col2:
            ...
            st.markdown("""
                <div style="text-align:center;background:white;border-radius:20px;padding:48px 44px;box-shadow:0 8px 32px rgba(0,0,0,0.12);">
                    <div style="color:#D11031;font-size:38px;font-weight:900;letter-spacing:3px;margin-bottom:4px;">HOLLYS</div>
                    <div style="color:#999;font-size:13px;letter-spacing:1px;">Roasting Center QMS</div>
                </div>
            """, unsafe_allow_html=True)
            st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
            pw = st.text_input("비밀번호", type="password", placeholder="비밀번호를 입력하세요", key="login_pw", label_visibility="collapsed")
            if st.button("로그인", use_container_width=True, type="primary", key="login_btn"):
                try:
                    pw_admin  = st.secrets["PASSWORD_ADMIN"]
                    pw_worker = st.secrets["PASSWORD_WORKER"]
                    pw_part   = st.secrets["PASSWORD_PART"]
                except:
                    pw_admin, pw_worker, pw_part = "0564", "0567", "1111"
                if pw == pw_admin:
                    ...
                    st.session_state.logged_in = True
                    st.session_state.role = "parttime"
                    st.rerun()
                else:
...
DESTINATION_FILE   = "destinations.csv"       # 출하처 마스터
PALLET_MASTER_FILE = "pallet_master.csv"      # 파렛트 마스터
PALLET_LOG_FILE    = "pallet_logs.csv"        # 파렛트 입출고 로그
...
def calc_annual_leave(hire_date_str, leave_records_df, emp_id, today=None):
    ...
    try:
        hire_ts = pd.to_datetime(hire_date_str, errors="coerce")
        if hire_ts is None or pd.isna(hire_ts):
            return 0, 0, 0, {}
        hire = hire_ts.date()
        if hire >= today:
            return 0, 0, 0, {}
...
def get_destinations():
    if os.path.exists(DESTINATION_FILE):
        try:
            df = pd.read_csv(DESTINATION_FILE, dtype=str).fillna("")
            if "출하처명" in df.columns:
                df = df.rename(columns={"출하처명": "업체명"})
            for c in ["업체명","담당자","연락처","주소","비고"]:
                if c not in df.columns: df[c] = ""
            return df
        except Exception: pass
    return pd.DataFrame(columns=["업체명","담당자","연락처","주소","비고"])
...
def save_pallet_logs(df):
    ...
    log_history("파렛트 로그 업데이트", "출고 관리", f"데이터 {len(df)}건 저장")
...
with st.sidebar:
    ...
    # 역할별 메뉴 필터링
    _role = st.session_state.get("role") or "admin"
    if _role == "admin":
        _menu_options = ["대시보드 (메인)", "게시판", "캘린더", "제품 관리", "원·부자재 관리", "재고 관리", "출고 관리", "직원 관리", "설비 관리", "HACCP", "시스템 히스토리"]
        _role_label, _role_color = "🔴 관리자", "#D11031"
    elif _role == "worker":
        _menu_options = ["대시보드 (메인)", "게시판", "캘린더", "제품 관리", "원·부자재 관리", "재고 관리", "출고 관리", "직원 관리", "설비 관리"]
        _role_label, _role_color = "🔵 중간관리자", "#2E75B6"
    else:
        _menu_options = ["대시보드 (메인)", "직원 관리"]
        _role_label, _role_color = "🟢 파트타이머", "#217346"

    st.markdown(f"<div style='background:white;border-radius:8px;padding:8px 12px;margin-bottom:10px;border-left:4px solid {_role_color};'><span style='font-size:13px;font-weight:700;color:{_role_color};'>{_role_label}</span></div>", unsafe_allow_html=True)
    ...
    if menu_selection == "캘린더":
        ...
        if _role == "parttime":
            sub_menu = "파트타이머 연차 관리"
            st.markdown("<span style='font-size:14px;color:#217346;font-weight:600;'>📋 파트타이머 연차 관리</span>", unsafe_allow_html=True)
        else:
        ...
        sub_menu = st.radio("하위 메뉴 선택:", ["제품 등록", "제품 규격 마스터", "생산 제품 품질 검사", "데이터 히스토리", "제품별 데이터 관리"])
        ...
        st.divider()
        st.markdown("#### 🚚 출고 관리")
        sub_menu = st.radio("하위 메뉴 선택:", ["출고등록", "파렛트 관리", "출하처 관리"])
        ...
        sub_menu = st.radio("하위 메뉴 선택:", ["HACCP 일지", "HACCP 기준서", "개정이력", "공정 흐름도", "공정별 CCP 결정도", "원부자재 CCP 결정도", "심각성 설정", "공정 위해요소분석", "원부자재 위해요소분석", "스마트 위해요소 설정"])
...
# --- 제품 관리 메뉴 ---
    ...
    TD_P  = "background:#fff;padding:8px 8px;border:1px solid #d0d7de;vertical-align:middle;font-size:11px;text-align:center;"
    ...
    if sub_menu == "제품 등록":
        st.markdown('<div class="section-title">📦 제품 규격 등록 및 상세 정보</div>', unsafe_allow_html=True)
        df_specs = load_specs()

        with st.expander("➕ 새 제품 등록 / 정보 업데이트", expanded=False):
            with st.form("spec_f"):
                s_c1, s_c2 = st.columns([1, 1.5])
                with s_c1:
                    new_p_code = st.text_input("제품코드 (예: HL-001)")
                    new_p_name = st.text_input("제품명")
                    new_p_type = st.selectbox("유형", ["캡슐커피", "스틱커피", "원두커피", "생두(원료)"])
                    new_p_spec = st.text_input("규격 (콤마 구분)", placeholder="100g, 200g")
                with s_c2:
                    st.caption("분석 항목별 판정 기준")
                    a_c1, a_c2 = st.columns(2)
                    with a_c1:
                        min_n2 = st.number_input("질소 최소 (%)", value=97.0)
                        max_n2 = st.number_input("질소 최대 (%)", value=100.0)
                        min_moi = st.number_input("수분 최소 (%)", value=0.0)
                    with a_c2:
                        max_moi = st.number_input("수분 최대 (%)", value=5.0)
                        min_ext = st.number_input("추출 최소 (초)", value=22.0)
                        max_ext = st.number_input("추출 최대 (초)", value=28.0)
                if st.form_submit_button("제품 정보 저장"):
                    if new_p_code and new_p_name:
                        new_row = pd.DataFrame([[new_p_code, new_p_name, new_p_type, new_p_spec, "", min_n2, max_n2, min_moi, max_moi, "N/A", "N/A", min_ext, max_ext, "해당없음", date.today()]], columns=df_specs.columns)
                        df_specs = pd.concat([df_specs[df_specs["제품코드"] != new_p_code], new_row], ignore_index=True)
                        df_specs.to_csv(SPEC_FILE, index=False, encoding='utf-8-sig'); st.success("저장 완료!"); st.rerun()

        st.markdown("#### 📋 등록 제품 현황")
        st.data_editor(df_specs, num_rows="dynamic", use_container_width=True, key="spec_editor_main")


    # ════════════════════════════════════════════════════════
    # 2. 생산 제품 품질 검사 (신규 메뉴: 측정기록 입력 전용)
    # ════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════
        ...
        df_specs = load_specs()
        top1, top2, top3, top4 = st.columns([2, 2, 3, 1.5])
        with top1:
        ...
        with top2:
            if df_specs.empty:
                ...
                type_list = ["전체"] + sorted(df_specs["유형"].dropna().unique().tolist())
                sel_type = st.selectbox("유형 선택", type_list, key="hist_type_pick")
        ...
        with top3:
            if df_specs.empty:
                ...
                if sel_type == "전체":
                    p_list = df_specs["제품명"].tolist()
                else:
                    p_list = df_specs[df_specs["유형"] == sel_type]["제품명"].tolist()
                add_prod = st.selectbox("제품 선택", [""] + p_list, key="hist_prod_pick")
        with top4:
    ...
    elif sub_menu == "생산 제품 품질 검사":
        st.subheader("🧪 생산 제품 품질 검사 및 판정")
        df_specs = load_specs()
        
        qa_date = st.date_input("📅 검사 대상 생산일 선택", value=date.today(), key="qa_date_pick")
        qa_date_str = str(qa_date)
        
        # 현재 날짜에 생산된 제품들 (데이터 히스토리에서 등록된 것들)
        prod_list = df[df["생산일"] == qa_date_str].copy().reset_index()
        
        if prod_list.empty:
            st.info(f"📅 {qa_date_str} 에 등록된 생산 기록이 없습니다. '데이터 히스토리'에서 먼저 등록해 주세요.")
        else:
            st.markdown(f"#### 📦 {qa_date_str} 생산 목록 (총 {len(prod_list)}건)")
            for ri, row in prod_list.iterrows():
                pname = row["제품명"]; p_idx = row["index"]
                p_status = str(row.get("판정", "미검사")).replace("nan", "미검사")
    # ════════════════════════════════════════════════════════
    # 4. 제품별 데이터 관리 (트렌드 조회 전용)
    # ════════════════════════════════════════════════════════
    elif sub_menu == "제품별 데이터 관리":
        st.markdown('<div class="section-title">📈 제품별 품질 데이터 트렌드</div>', unsafe_allow_html=True)
        if df.empty:
            st.warning("등록된 데이터 히스토리가 없습니다.")
        else:
            p_list = sorted([p for p in df['제품명'].dropna().unique().tolist() if str(p).strip() not in ["","-","None","nan"]])
            if not p_list:
                st.info("조회할 수 있는 제품 데이터가 없습니다.")
            else:
                fc1, fc2 = st.columns([2,3])
                with fc1:
                    sel_p = st.selectbox("🔍 제품 선택", p_list)
                with fc2:
                    cat_map = {"질소":"질소(%)","수분":"수분(%)","색도":"색도(Agtron)","추출시간":"추출시간(sec)"}
                    sel_cat = st.radio("📊 지표 선택", list(cat_map.keys()), horizontal=True)
                
                t_col = cat_map[sel_cat]
                p_df = df[df['제품명']==sel_p].copy()
                p_df['생산일'] = pd.to_datetime(p_df['생산일'], errors='coerce')
                p_df = p_df.dropna(subset=['생산일']).sort_values('생산일')
                p_df[t_col] = pd.to_numeric(p_df[t_col], errors='coerce')
                chart_df = p_df.dropna(subset=[t_col]).copy()
                chart_df['date_str'] = chart_df['생산일'].dt.strftime('%Y-%m-%d')

                if chart_df.empty:
                    st.info(f"'{sel_p}'의 '{sel_cat}' 측정 데이터가 없습니다.")
                else:
                    vals = chart_df[t_col]
                    cols = st.columns(4)
                    cols[0].metric("평균", f"{vals.mean():.2f}")
                    cols[1].metric("최소", f"{vals.min():.2f}")
                    cols[2].metric("최대", f"{vals.max():.2f}")
                    cols[3].metric("측정수", f"{len(vals)}회")
                    
                    st.markdown(f"**📉 {sel_p} — {sel_cat} 트렌드**")
                    import plotly.express as px
                    fig = px.line(chart_df, x='date_str', y=t_col, markers=True, text=t_col, color_discrete_sequence=['#D11031'])
                    fig.update_traces(textposition="top center")
                    st.plotly_chart(fig, use_container_width=True)

                    with st.expander("📋 상세 데이터 및 엑셀 다운로드"):
                        st.dataframe(chart_df[['date_str', t_col, '판정', '비고']], use_container_width=True)
                        
                        def export_trend(data, pn, cn):
                            import io
                            out = io.BytesIO()
                            with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
                                data.to_excel(writer, index=False, sheet_name='Trend')
                            return out.getvalue()
                        
                        st.download_button(
                            label=f"📥 {sel_cat} 데이터 엑셀 다운로드",
                            data=export_trend(chart_df[['date_str', t_col, '판정', '비고']], sel_p, sel_cat),
                            file_name=f"Hollys_{sel_p}_{sel_cat}_{date.today().strftime('%Y%m%d')}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True)
...
# 🌟 5. 직원 관리 메뉴 
    ...
    if sub_menu == "조직도 및 인원 관리":
        ...
        pt_options = {f"{r['이름']} ({r['사번']})": i for i, r in pt_df.iterrows()}
        sel_label  = st.selectbox("👤 파트타이머 선택", list(pt_options.keys()), key="pt_sel")
        sel_idx    = pt_options[sel_label]
        sel_row    = pt_df.iloc[sel_idx]
        sel_empid  = str(sel_row.get("사번", "")).strip()
        if sel_empid.lower() in ("nan", "none", ""):
            sel_empid = f"__name__{sel_row.get('이름','')}"
        ...
        with tab_add:
            ...
            with fa1:
                f_date = st.date_input("날짜 *", value=today, key=f"lv_add_date_{sel_idx}")
            with fa2:
                f_type = st.selectbox("유형 *", ["연차", "무급휴가"], key=f"lv_add_type_{sel_idx}")
            f_note = st.text_input("비고", placeholder="예: 개인 사정, 병가 등", key=f"lv_add_note_{sel_idx}")

            if f_type == "무급휴가":
                st.warning("⚠️ 무급휴가 등록 시, 해당 월의 연차 1개가 발생하지 않습니다.")

            if st.button("등록", type="primary", use_container_width=True, key="lv_add_btn"):
                st.session_state["lv_confirm_pending"] = {
                    "empid": sel_empid,
                    "name":  sel_row["이름"],
                    "date":  f_date,
                    "type":  f_type,
            ...
            pending = st.session_state.get("lv_confirm_pending")
            if pending and pending.get("empid") == sel_empid:
                note_str = f" | {pending['note']}" if pending['note'] else ""
                st.warning(f"📝 **{pending['date']}  |  {pending['type']}{note_str}** 을(를) 등록하시겠습니까?")
                cy, cn = st.columns(2)
                with cy:
                    if st.button("✅ 예, 등록합니다", type="primary", use_container_width=True, key="lv_confirm_yes"):
                        new_rec = pd.DataFrame([{
                            "사번": pending["empid"],
                            "이름": pending["name"],
                            "날짜": pending["date"].strftime("%Y-%m-%d"),
                            "유형": pending["type"],
                            "비고": pending["note"],
                        }])
                        df_lv_saved = load_annual_leave()
                        df_lv_saved = pd.concat([df_lv_saved, new_rec], ignore_index=True)
                        save_annual_leave(df_lv_saved)
                        del st.session_state["lv_confirm_pending"]
                        st.success(f"✅ [{pending['date']}] {pending['type']} 등록 완료!")
                        st.rerun()
                with cn:
                    if st.button("❌ 아니오, 취소", use_container_width=True, key="lv_confirm_no"):
                        del st.session_state["lv_confirm_pending"]
                        st.rerun()
        ...
        st.markdown('<div class="section-title">🏥 보건증 자동 관리 시스템</div>', unsafe_allow_html=True)
        st.write("직원별 보건증 갱신 주기를 자동으로 계산하고 엑셀 파일로 출력할 수 있습니다.")

        df_hc = load_health_cert()
        df_emp = load_employees()
        active_emps = df_emp[df_emp['재직상태'] == '재직'].copy()
        
        with st.expander("➕ 신규 보건증 등록"):
...
# --- 출고 관리 메뉴 ---
    ...
    if sub_menu == "출고등록":
        ...
        with st.expander("➕ 새 출고 배차 등록", expanded=True):
            # 출하처 목록 로드
            _df_dest_ob = get_destinations()
            _dest_names_ob = _df_dest_ob["업체명"].dropna().tolist() if not _df_dest_ob.empty else []
            ...
            with c2:
                if _dest_names_ob:
                    _dest_sel_ob = st.selectbox("출하처 *", ["선택하세요"] + _dest_names_ob, key="ob_dest_sel")
                    out_dest = "" if _dest_sel_ob == "선택하세요" else _dest_sel_ob
                    # 주소 표시
                    if out_dest:
                        _dest_row_ob = _df_dest_ob[_df_dest_ob["업체명"] == out_dest]
                        if not _dest_row_ob.empty:
                            _addr_ob = str(_dest_row_ob.iloc[0].get("주소", "")).strip()
                            _tel_ob  = str(_dest_row_ob.iloc[0].get("연락처", "")).strip()
                            _info_parts = []
                            if _addr_ob: _info_parts.append(f"📍 {_addr_ob}")
                            if _tel_ob:  _info_parts.append(f"📞 {_tel_ob}")
                            if _info_parts:
                                st.caption("  |  ".join(_info_parts))
                else:
                    out_dest = st.text_input("출하처 *", placeholder="출하처 관리 메뉴에서 먼저 등록하세요")
                    st.caption("출하처 관리 메뉴에서 출하처를 등록하면 자동 연동됩니다.")
            out_time = ""  # 출고시간 미사용
            
            st.write("---")
            st.markdown("**📦 출하 제품 및 수량**")
            st.caption("재고가 있는 배치가 표시됩니다. **출고수량** 칸에 숫자를 입력하세요. (0이면 미출고)")

            # ── 배치별 재고 계산 (제품명+규격+생산일+소비기한 기준) ──
            ...
            for _, row in df_adj_for_inv.iterrows():
                ...
                if pd_ and pd_ != "-":
                    bkey = (pn, spec, pd_, exp_)
                    batch_adj[bkey] = batch_adj.get(bkey, 0) + diff
                else:
                    prod_adj_unmatched[pn] = prod_adj_unmatched.get(pn, 0) + diff
            ...
            for bkey, pqty in batch_prod.items():
                ...
                aqty = batch_adj.get(bkey, 0)
                batch_stock[bkey] = pqty - oqty + aqty
            for pn, adj in prod_adj_unmatched.items():
                matching_batches = [k for k in batch_stock.keys() if k[0] == pn]
                if matching_batches:
                    last_b = sorted(matching_batches, key=lambda x: x[2])[-1]
                    batch_stock[last_b] = batch_stock.get(last_b, 0) + adj

            # ── 제품명+규격 → 규격코드 매핑 ──
            _spec_code_map_all = {}
            if not df_p_list.empty and "규격코드" in df_p_list.columns:
                for _, _sr in df_p_list.iterrows():
                    _k = (str(_sr.get("제품명","")).strip(), str(_sr.get("규격","")).strip())
                    _v = str(_sr.get("규격코드","")).strip()
                    if _v: _spec_code_map_all[_k] = _v

            # ── 유형 매핑 (제품명 → 유형) ──
            _prod_type_map = {}
            if not df_p_list.empty:
                for _, _sr in df_p_list.iterrows():
                    _pn2 = str(_sr.get("제품명","")).strip()
                    _pt2 = str(_sr.get("유형","")).strip()
                    if _pn2: _prod_type_map[_pn2] = _pt2

            # ── 엑셀형 데이터 그리드 구성 ──
            _grid_rows = []
            for bkey, remain in sorted(batch_stock.items(), key=lambda x: (x[0][0], x[0][2])):
                if int(remain) <= 0: continue
                _rc = _spec_code_map_all.get((bkey[0], bkey[1]), "")
                _grid_rows.append({
                    "✓": False,
                    "유형": _prod_type_map.get(bkey[0], ""),
                    "제품명": bkey[0],
                    "규격코드": _rc,
                    "규격": bkey[1],
                    "제조일자": bkey[2],
            ...
            if not _grid_rows:
                st.info("현재 재고가 있는 배치가 없습니다.")
                out_items = []
                _de_result = pd.DataFrame()
            else:
                _grid_df = pd.DataFrame(_grid_rows)
                _de_result = st.data_editor(
                    _grid_df,
                    column_config={
                        "✓": st.column_config.CheckboxColumn("✓", width=30, help="출고 대상 체크"),
                        "유형": st.column_config.TextColumn("유형", disabled=True, width="small"),
                        "제품명": st.column_config.TextColumn("제품명", disabled=True, width="large"),
                        "규격코드": st.column_config.TextColumn("규격코드", disabled=True, width="small"),
                        "규격": st.column_config.TextColumn("규격", disabled=True, width="medium"),
                        "제조일자": st.column_config.TextColumn("제조일자", disabled=True, width="small"),
                        "소비기한": st.column_config.TextColumn("소비기한", disabled=True, width="small"),
                        "현재고": st.column_config.NumberColumn("현재고", disabled=True, width="small", format="%d 개"),
                        "출고수량": st.column_config.NumberColumn("출고수량", min_value=0, step=1, width="small", format="%d 개"),
                ...
                out_items = []
                for _, _er in _de_result.iterrows():
                    _qty = int(_er.get("출고수량", 0) or 0)
                    _pn = str(_er.get("제품명", "")).strip()
                    if _pn and _qty > 0:
                        out_items.append({
                            "유형": str(_er.get("유형", "")).strip() or "-",
                            "제품명": _pn,
                            "규격": str(_er.get("규격", "")).strip() or "-",
                            "제조일자": str(_er.get("제조일자", "")).strip(),
                            "소비기한": str(_er.get("소비기한", "")).strip(),
                            "수량": str(_qty),
                            "비고": str(_er.get("비고", "")).strip()
                        })

            # ── 파렛트 출고 섹션 ──────────────────────────────────────────
            st.markdown("### 🪵 파렛트 출고 *(선택사항)*")
            _ob_pm = get_pallet_master()
            _ob_pl = get_pallet_logs()

            def _ob_calc_stock(pm, pl):
                s = {r["파렛트명"]: 0 for _, r in pm.iterrows()}
                for _, r in pl.iterrows():
                    nm = str(r.get("파렛트명","")).strip()
                    try: q = int(str(r.get("수량","0")).strip() or 0)
                    except: q = 0
                    if nm in s:
                        s[nm] = s[nm] + q if str(r.get("유형","")).strip() == "입고" else s[nm] - q
                return s

            if _ob_pm.empty:
                st.info("파렛트 마스터에 등록된 파렛트가 없습니다. 파렛트 관리 메뉴에서 먼저 등록하세요.")
                _ob_pallet_items = {}
            else:
                _ob_stock = _ob_calc_stock(_ob_pm, _ob_pl)
                # 파렛트 카드 + 수량 입력 그리드
                _ob_pcols = st.columns(min(len(_ob_pm), 4))
                _ob_pallet_items = {}
                for _opi, (_, _opr) in enumerate(_ob_pm.iterrows()):
                    _opnm  = str(_opr.get("파렛트명",""))
                    _opcol = str(_opr.get("색상","#2E75B6"))
                    _opqty = _ob_stock.get(_opnm, 0)
                    with _ob_pcols[_opi % 4]:
                        st.markdown(
                            f"<div style='background:{_opcol}22;border:2px solid {_opcol};border-radius:10px;"
                            f"padding:10px 8px 4px 8px;text-align:center;margin-bottom:4px;'>"
                            f"<div style='font-size:12px;font-weight:bold;color:{_opcol};'>{_opnm}</div>"
                            f"<div style='font-size:22px;font-weight:bold;color:{_opcol};'>{_opqty}</div>"
                            f"<div style='font-size:10px;color:#888;'>현재 재고</div></div>",
                            unsafe_allow_html=True
                        )
                        _ob_inp = st.number_input(
                            f"출고수량", min_value=0,
                            value=0, step=1, key=f"ob_pal_{_opnm}",
                            help=f"현재 재고: {_opqty}개"
                        )
                        if _ob_inp > 0:
                            _ob_pallet_items[_opnm] = _ob_inp

            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🚀 출고 데이터 일괄 등록", type="primary", use_container_width=True):
                if not out_dest:
                    st.error("출하처는 필수 입력 사항입니다.")
                elif not out_items and not _ob_pallet_items:
                    st.error("최소 하나 이상의 제품 또는 파렛트 수량(0 초과)을 입력해야 합니다.")
                else:
                    # 제품 출고 저장
                    if out_items:
                        new_rows = []
                    ...
                    # 파렛트 출고 저장
                    if _ob_pallet_items:
                        _ob_pl_new = get_pallet_logs()
                        _new_pal_rows = []
                        for _pnm, _pqty in _ob_pallet_items.items():
                            _new_pal_rows.append({
                                "날짜": str(out_date),
                                "유형": "출고",
                                "파렛트명": _pnm,
                                "수량": str(_pqty),
                                "비고": f"출고등록 – {out_dest} ({out_time})"
                            })
                        _ob_pl_new = pd.concat([_ob_pl_new, pd.DataFrame(_new_pal_rows)], ignore_index=True)
                        save_pallet_logs(_ob_pl_new)
                        log_history("파렛트 출고", "출고 관리", f"출하처: {out_dest}, 파렛트 {len(_new_pal_rows)}종 출고")

                    _total = len(out_items) + len(_ob_pallet_items)
                    _msg_parts = []
                    if out_items: _msg_parts.append(f"제품 {len(out_items)}건")
                    if _ob_pallet_items: _msg_parts.append(f"파렛트 {len(_ob_pallet_items)}종")
                    st.success(f"✅ {' + '.join(_msg_parts)} 출고 등록 완료!")
                    st.rerun()
        ...
        if df_out.empty:
            ...
            if df_filtered.empty:
                ...
                if view_mode == "당일 전체 출고조회":
                    ...
                    # 개별 배차 선택 (출하처 기준)
                    배차목록 = df_filtered["출하처"].dropna().unique().tolist()
                    if 배차목록:
                        sel_배차 = st.selectbox("배차(출하처) 선택", 배차목록)
                        target_df = df_filtered[df_filtered["출하처"] == sel_배차]
                        file_name_prefix = f"Hollys_Outbound_Car_{search_date.strftime('%Y%m%d')}_{sel_배차}"
                        doc_title = f"출고 명세서"
    ...
    # ── 파렛트 관리 ──────────────────────────────────────────────────────
    elif sub_menu == "파렛트 관리":
        st.markdown('<div class="section-title">🪵 파렛트 관리</div>', unsafe_allow_html=True)

        _df_pm = get_pallet_master()
        _df_pl = get_pallet_logs()

        # 현재 재고 계산
        def _calc_pallet_stock(pm, pl):
            stock = {row["파렛트명"]: 0 for _, row in pm.iterrows()}
            for _, row in pl.iterrows():
                nm = str(row.get("파렛트명","")).strip()
                qty = int(str(row.get("수량","0")).strip() or 0)
                if nm in stock:
                    if str(row.get("유형","")).strip() == "입고":
                        stock[nm] += qty
                    else:
                        stock[nm] -= qty
            return stock

        _stock = _calc_pallet_stock(_df_pm, _df_pl) if not _df_pm.empty else {}

        # 파렛트 현황 카드
        if not _df_pm.empty:
            st.markdown("#### 📊 파렛트 현재 재고")
            _pcols = st.columns(min(len(_df_pm), 4))
            for _pi, (_, _pr) in enumerate(_df_pm.iterrows()):
                _pnm = str(_pr.get("파렛트명",""))
                _pcol = str(_pr.get("색상","#2E75B6"))
                _pqty = _stock.get(_pnm, 0)
                _pcols[_pi % 4].markdown(
                    f"<div style='background:{_pcol}22;border:2px solid {_pcol};border-radius:10px;padding:14px;text-align:center;margin:4px;'>"
                    f"<div style='font-size:12px;font-weight:bold;color:{_pcol};'>{_pnm}</div>"
                    f"<div style='font-size:28px;font-weight:bold;color:{_pcol};'>{_pqty}</div>"
                    f"<div style='font-size:11px;color:#888;'>개</div></div>",
                    unsafe_allow_html=True
                )
            st.markdown("<br>", unsafe_allow_html=True)
        else:
            st.info("등록된 파렛트가 없습니다. 아래에서 먼저 파렛트를 등록하세요.")

        _tab1, _tab2, _tab3 = st.tabs(["입출고 등록", "입출고 이력", "파렛트 마스터 관리"])

        with _tab1:
            st.markdown("#### ➕ 파렛트 입출고 등록")
            if _df_pm.empty:
                st.warning("먼저 '파렛트 마스터 관리' 탭에서 파렛트를 등록하세요.")
            else:
                _pnames = _df_pm["파렛트명"].tolist()
                _c1, _c2, _c3 = st.columns(3)
                with _c1:
                    _p_date = st.date_input("날짜", value=date.today(), key="pl_date")
                    _p_type = st.selectbox("유형", ["입고", "출고"], key="pl_type")
                with _c2:
                    _p_name = st.selectbox("파렛트", _pnames, key="pl_name")
                    _p_qty  = st.number_input("수량", min_value=1, value=1, key="pl_qty")
                with _c3:
                    _p_note = st.text_input("비고", key="pl_note", placeholder="출하처명, 목적 등")
                if st.button("등록", type="primary", use_container_width=True, key="pl_save"):
                    _new_pl = pd.concat([_df_pl, pd.DataFrame([{
                        "날짜": str(_p_date), "유형": _p_type,
                        "파렛트명": _p_name, "수량": str(_p_qty), "비고": _p_note
                    }])], ignore_index=True)
                    save_pallet_logs(_new_pl)
                    st.success(f"✅ {_p_name} {_p_type} {_p_qty}개 등록 완료!")
                    st.rerun()

            # ── 영점 맞추기 ───────────────────────────────────────────────
            st.markdown("---")
            st.markdown("#### 🎯 영점 맞추기 (실사 기준 재고 맞춤)")
            st.caption("실제 재고 수량을 입력하면 전산 재고와의 차이를 조정 항목으로 자동 등록합니다.")
            if _df_pm.empty:
                st.info("등록된 파렛트가 없습니다.")
            else:
                _zc1, _zc2 = st.columns(2)
                with _zc1:
                    _z_date = st.date_input("기준일", value=date.today(), key="pz_date")
                    _z_name = st.selectbox("파렛트 선택", _df_pm["파렛트명"].tolist(), key="pz_name")
                with _zc2:
                    _z_cur = _stock.get(_z_name, 0)
                    st.text_input("현재 전산 재고", value=f"{_z_cur} 개", disabled=True, key="pz_cur_disp")
                    _z_actual = st.number_input("실사 수량", min_value=0, value=max(_z_cur, 0), step=1, key="pz_actual")
                _z_diff = _z_actual - _z_cur
                if _z_diff > 0:
                    st.info(f"📥 **+{_z_diff}개 입고** 조정이 등록됩니다. (실사 {_z_actual} - 전산 {_z_cur})")
                elif _z_diff < 0:
                    st.warning(f"📤 **{_z_diff}개 출고** 조정이 등록됩니다. (실사 {_z_actual} - 전산 {_z_cur})")
                else:
                    st.success(f"✅ 전산과 실사 수량이 일치합니다. ({_z_cur}개)")
                if st.button("🎯 영점 맞추기 적용", key="pz_apply", disabled=(_z_diff == 0)):
                    _z_type = "입고" if _z_diff > 0 else "출고"
                    _z_qty  = abs(_z_diff)
                    _z_note = f"영점 맞추기 – 실사 {_z_actual}개 / 전산 {_z_cur}개"
                    _new_pl_z = pd.concat([_df_pl, pd.DataFrame([{
                        "날짜": str(_z_date), "유형": _z_type,
                        "파렛트명": _z_name, "수량": str(_z_qty), "비고": _z_note
                    }])], ignore_index=True)
                    save_pallet_logs(_new_pl_z)
                    st.success(f"✅ {_z_name} 재고 조정 완료! ({_z_cur}개 → {_z_actual}개)")
                    st.rerun()
        ...
        with _tab2:
            ...
            if _df_pl.empty:
                ...
                _df_pl_show = _df_pl.copy()
                _c1, _c2 = st.columns(2)
                with _c1:
                    _f_type = st.selectbox("유형 필터", ["전체","입고","출고"], key="pl_ftype")
                with _c2:
                    _f_pname = st.selectbox("파렛트 필터", ["전체"] + (_df_pm["파렛트명"].tolist() if not _df_pm.empty else []), key="pl_fname")
                if _f_type != "전체":
                    _df_pl_show = _df_pl_show[_df_pl_show["유형"] == _f_type]
                if _f_pname != "전체":
                    _df_pl_show = _df_pl_show[_df_pl_show["파렛트명"] == _f_pname]
                st.dataframe(_df_pl_show.sort_values("날짜", ascending=False), use_container_width=True, hide_index=True)
                # 삭제
                st.caption("삭제:")
                for _li, _lr in _df_pl_show.iterrows():
                    _dc1, _dc2 = st.columns([6, 0.8])
                    with _dc1: st.caption(f"{_lr.get('날짜','')} | {_lr.get('유형','')} | {_lr.get('파렛트명','')} | {_lr.get('수량','')}개 | {_lr.get('비고','')}")
                    with _dc2:
                        if st.button("🗑️", key=f"pldel_{_li}", use_container_width=True):
                            save_pallet_logs(_df_pl.drop(index=_li).reset_index(drop=True))
                            st.success("삭제 완료"); st.rerun()

        with _tab3:
            st.markdown("#### ⚙️ 파렛트 마스터 관리")
            if not _df_pm.empty:
                st.dataframe(_df_pm, use_container_width=True, hide_index=True)
            st.markdown("##### ➕ 파렛트 추가")
            _mc1, _mc2, _mc3 = st.columns([3, 2, 1])
            with _mc1:
                _new_pname = st.text_input("파렛트명", placeholder="예: N11(1100*1100)", key="pm_name")
            with _mc2:
                _new_pcol  = st.color_picker("색상", value="#2E75B6", key="pm_color")
            with _mc3:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("추가", use_container_width=True, key="pm_add"):
                    if _new_pname.strip():
                        _new_pm = pd.concat([_df_pm, pd.DataFrame([{"파렛트명": _new_pname.strip(), "색상": _new_pcol}])], ignore_index=True)
                        save_pallet_master(_new_pm)
                        st.success(f"✅ {_new_pname} 추가 완료!"); st.rerun()
                    else:
                        st.error("파렛트명을 입력하세요.")
            if not _df_pm.empty:
                st.markdown("##### 🗑️ 파렛트 삭제")
                _del_pname = st.selectbox("삭제할 파렛트", _df_pm["파렛트명"].tolist(), key="pm_del_sel")
                if st.button("삭제", type="secondary", key="pm_del_btn"):
                    save_pallet_master(_df_pm[_df_pm["파렛트명"] != _del_pname].reset_index(drop=True))
                    st.success(f"✅ {_del_pname} 삭제 완료!"); st.rerun()

    # ── 출하처 관리 ──────────────────────────────────────────────────────
    elif sub_menu == "출하처 관리":
        st.markdown('<div class="section-title">🏢 출하처 관리</div>', unsafe_allow_html=True)

        _df_dest = get_destinations()
        _DEST_COLS = ["업체명", "담당자", "연락처", "주소", "비고"]

        if "dest_mode" not in st.session_state: st.session_state.dest_mode = "list"
        if "dest_edit_idx" not in st.session_state: st.session_state.dest_edit_idx = None

        if st.session_state.dest_mode == "list":
            if st.button("➕ 출하처 추가", type="primary", key="dest_add_btn"):
                st.session_state.dest_mode = "add"; st.session_state.dest_edit_idx = None; st.rerun()

            if _df_dest.empty:
                st.info("등록된 출하처가 없습니다.")
            else:
                # 테이블 표시 + 수정/삭제
                TH_D = "background:#1F4E79;color:white;font-weight:bold;padding:8px 10px;border:1px solid #144070;font-size:12px;"
                TD_D = "padding:8px 10px;border:1px solid #d0d7de;font-size:12px;vertical-align:middle;"
                _dhtml = (f"<style>.desttbl{{width:100%;border-collapse:collapse;}}</style>"
                          f"<table class='desttbl'><thead><tr>"
                          f"<th style='{TH_D}'>업체명</th><th style='{TH_D}'>담당자</th>"
                          f"<th style='{TH_D}'>연락처</th><th style='{TH_D}'>주소</th><th style='{TH_D}'>비고</th>"
                          f"</tr></thead><tbody>")
                for _, _dr in _df_dest.iterrows():
                    _dhtml += (f"<tr><td style='{TD_D}font-weight:bold;'>{_dr.get('업체명','')}</td>"
                               f"<td style='{TD_D}'>{_dr.get('담당자','')}</td>"
                               f"<td style='{TD_D}'>{_dr.get('연락처','')}</td>"
                               f"<td style='{TD_D}'>{_dr.get('주소','')}</td>"
                               f"<td style='{TD_D}'>{_dr.get('비고','')}</td></tr>")
                _dhtml += "</tbody></table>"
                st.markdown(_dhtml, unsafe_allow_html=True)
                st.markdown("<br>", unsafe_allow_html=True)
                st.caption("수정·삭제:")
                for _di, _dr in _df_dest.iterrows():
                    _dc1, _dc2, _dc3 = st.columns([6, 0.8, 0.8])
                    with _dc1: st.caption(f"**{_dr.get('업체명','')}** | {_dr.get('담당자','')} | {_dr.get('연락처','')}")
                    with _dc2:
                        if st.button("✏️", key=f"destedit_{_di}", use_container_width=True):
                            st.session_state.dest_mode = "edit"; st.session_state.dest_edit_idx = _di; st.rerun()
                    with _dc3:
                        if st.button("🗑️", key=f"destdel_{_di}", use_container_width=True):
                            save_destinations(_df_dest.drop(index=_di).reset_index(drop=True))
                            st.success("삭제 완료"); st.rerun()

        else:  # add / edit
            _is_edit_d = (st.session_state.dest_mode == "edit")
            _eidx_d    = st.session_state.dest_edit_idx
            def _dv(col):
                if _is_edit_d and _eidx_d is not None and not _df_dest.empty:
                    try: return str(_df_dest.iloc[_eidx_d].get(col,""))
                    except: pass
                return ""
            st.markdown(f"#### {'✏️ 출하처 수정' if _is_edit_d else '➕ 출하처 추가'}")
            _da, _db = st.columns(2)
            with _da:
                _d_nm   = st.text_input("업체명 *", value=_dv("업체명"), key="dest_nm")
                _d_tel  = st.text_input("연락처", value=_dv("연락처"), key="dest_tel")
            with _db:
                _d_mgr  = st.text_input("담당자", value=_dv("담당자"), key="dest_mgr")
                _d_note = st.text_input("비고", value=_dv("비고"), key="dest_note")
            _d_addr = st.text_input("주소", value=_dv("주소"), key="dest_addr")
            _sb1, _sb2 = st.columns(2)
            with _sb1:
                if st.button("저장", type="primary", use_container_width=True, key="dest_save"):
                    if not _d_nm.strip():
                        st.error("업체명은 필수입니다.")
                    else:
                        _nr = {"업체명":_d_nm.strip(),"담당자":_d_mgr,"연락처":_d_tel,"주소":_d_addr,"비고":_d_note}
                        if _is_edit_d and _eidx_d is not None:
                            for _k,_v in _nr.items(): _df_dest.at[_eidx_d,_k]=_v
                        else:
                            _df_dest = pd.concat([_df_dest, pd.DataFrame([_nr])], ignore_index=True)
                        save_destinations(_df_dest)
                        st.session_state.dest_mode="list"; st.session_state.dest_edit_idx=None
                        st.success("✅ 저장 완료!"); st.rerun()
            with _sb2:
                if st.button("취소", use_container_width=True, key="dest_cancel"):
                    st.session_state.dest_mode="list"; st.session_state.dest_edit_idx=None; st.rerun()
...
# --- 10. HACCP 관리 메뉴 ---
    ...
    def load_rm_hazard():
        cols = ["원자재명","원자재코드","유형코드","유형","위해요소","위해요인","심각성","발생가능성","위해도","관리방안","최종위해요소","비고"]
        if os.path.exists(RM_HAZARD_FILE):
            try:
                df = pd.read_csv(RM_HAZARD_FILE, dtype=str).fillna("")
                for c in cols:
                    if c not in df.columns: df[c] = ""
                return df
            except Exception: pass
    ...
    CCP_DIR = "ccp_decisions"
    os.makedirs(CCP_DIR, exist_ok=True)

    def get_rm_ccp_decision(rm_code):
        safe = str(rm_code).replace("/","_").replace("\\","_").replace(" ","_")
        fpath = os.path.join(CCP_DIR, f"rm_ccp_decision_{safe}.csv")
        cols = ["단계명","위해유형","위해요소","Q1_답변","Q1_비고","Q2_답변","Q2_비고",
                "Q21_답변","Q21_비고","Q3_답변","Q3_비고","Q4_답변","Q4_비고",
                "Q5_답변","Q5_비고","CCP결정","CCP번호","비고"]
        if os.path.exists(fpath):
            try:
                df = pd.read_csv(fpath, dtype=str).fillna("")
                for c in cols:
    ...
    def save_rm_ccp_decision(rm_code, df):
        safe = str(rm_code).replace("/","_").replace("\\","_").replace(" ","_")
        fpath = os.path.join(CCP_DIR, f"rm_ccp_decision_{safe}.csv")
        df.to_csv(fpath, index=False, encoding="utf-8-sig")
        log_history("원부자재 CCP 결정도 업데이트", f"원부자재: {rm_code}", f"데이터 {len(df)}건 저장")

    def export_ccp_excel(df_d, cat_nm, step_no=None):
        if step_no is None:
            step_no = {}
        import io as _io
        from openpyxl import Workbook
...
- **합류 레인 2/3**: 동일 단계에 2~3개 레인이 동시 합류할 때 사용
                        """)
                        for col_need in ["lane", "merge_from", "merge_label", "merge_from_2", "merge_label_2", "merge_from_3", "merge_label_3", "CCP유형"]:
                        ...
                        cfg_flow = {
                            ...
                            "CCP유형":       st.column_config.TextColumn("CCP 유형 (예: B1, B2, P1, C1)", width="medium"),
                    ...
                    CCP_TYPE_COLOR = {
                        ...
                        "B,P":   "#6C2585",   # 짙은 빨강-보라
                        "C,P":   "#935116",   # 짙은 주황-보라
                        "B,C,P": "#1A5276",   # 네이비 - 모든 유형
                    }
                    CCP_COLOR   = "#D11031"   # 기본 CCP 색 (유형 미지정)
                    ARROW_COLOR = "#555555"
                    MERGE_COLOR = "#D11031"
                    LANE_BG     = [
                    ...
                    for ln in all_lanes:
                        ...
                        for si, (_, row) in enumerate(steps.iterrows()):
                            ...
                            ccp_유형  = str(row.get("CCP유형", "")).strip()

                            if is_ccp:
                                # B1, B2, P1 등 숫자 붙은 유형은 첫 글자 기준으로 색상 결정
                                _base_ccp = ccp_유형[0].upper() if ccp_유형 else ""
                                fill_c   = CCP_TYPE_COLOR.get(ccp_유형, CCP_TYPE_COLOR.get(_base_ccp, CCP_COLOR))
                                border_c = "#1A1A1A"
                            else:
                                fill_c   = TYPE_COLOR.get(유형, "#4A90D9")
                                border_c = "#2c2c2c"
                            ...
                            if is_ccp:
                                ccp_tag = f"[{ccp_유형}]" if ccp_유형 else ""
                                label += f" ⚠️{ccp_번호}{ccp_tag}"
                    ...
                    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:6px;">
                        <span style="background:#D11031;color:white;padding:3px 10px;border-radius:6px;font-size:0.82rem;">⚠️ CCP (유형미지정)</span>
                        <span style="background:#C0392B;color:white;padding:3px 10px;border-radius:6px;font-size:0.82rem;">⚠️ CCP-B (생물학적)</span>
                        <span style="background:#D35400;color:white;padding:3px 10px;border-radius:6px;font-size:0.82rem;">⚠️ CCP-C (화학적)</span>
                        <span style="background:#7D3C98;color:white;padding:3px 10px;border-radius:6px;font-size:0.82rem;">⚠️ CCP-P (물리적)</span>
                        <span style="background:#1A5276;color:white;padding:3px 10px;border-radius:6px;font-size:0.82rem;">⚠️ CCP-B,C,P</span>
        ...
        if df_cats.empty:
            ...
            if os.path.exists(HAZARD_FILE):
                try:
                    ...
                    # 현재 카테고리 + 종합평가 >= 3 필터 (최종위해요소 조건 없음)
                    df_haz_sync = df_haz_global[
                        (df_haz_global["카테고리"].astype(str).str.strip() == sel_cat_name.strip()) &
                        (pd.to_numeric(df_haz_global["종합평가"], errors="coerce").fillna(0) >= 3)
                    ]
...
| **질문5** | 확인된 위해요소를 제거하거나 그 발생을 허용수준으로 감소시킬 수 있는 이후의 공정이 있는가? | → CP | **→ CCP** |
            ...
            if st.session_state.ccp_mode == "list":
                col_add, col_opt, _ = st.columns([1, 1.4, 3.6])
                with col_add:
                    if st.button("항목 추가", type="primary", use_container_width=True):
                        ...
                        st.rerun()
                with col_opt:
                    if st.button("🧠 스마트 최적화", type="secondary", use_container_width=True, key="proc_ccp_smart_opt"):
                        # ── 1. 공정흐름도에서 CCP 단계 추출 ──
                        _ccp_steps_set = set()
                        if "CCP여부" in df_steps.columns:
                            _ccp_steps_set = set(
                                df_steps[df_steps["CCP여부"].astype(str).str.upper() == "Y"]["단계명"].str.strip().tolist()
                            )

                        # ── 2. 공정 위해요소분석에서 종합평가 >= 3 항목 수집 ──
                        _opt_rows = []
                        try:
                            _df_haz_opt = pd.read_csv(HAZARD_FILE, encoding="utf-8-sig").fillna("")
                            _df_haz_opt = _df_haz_opt[
                                (_df_haz_opt["카테고리"].astype(str).str.strip() == sel_cat_name.strip()) &
                                (pd.to_numeric(_df_haz_opt["종합평가"], errors="coerce").fillna(0) >= 3)
                            ]
                            for _, _hr in _df_haz_opt.iterrows():
                                _opt_rows.append({
                                    "단계명": str(_hr.get("공정명","")).strip(),
                                    "위해유형": str(_hr.get("유형","")).strip()[:1],
                                    "위해요소": str(_hr.get("위해요소","")).strip(),
                                })
                        except Exception:
                            pass

                        if not _opt_rows:
                            st.warning("공정 위해요소분석에서 종합평가 3점 이상 항목이 없습니다.")
                        else:
                            # ── 3. 기존 데이터 초기화 후 재구성 ──
                            DEC_COLS = ["단계명","위해유형","위해요소",
                                        "Q1_답변","Q1_비고","Q2_답변","Q2_비고","Q21_답변","Q21_비고",
                                        "Q3_답변","Q3_비고","Q4_답변","Q4_비고","Q5_답변","Q5_비고",
                                        "CCP결정","CCP번호","비고"]
                            df_opt = pd.DataFrame(columns=DEC_COLS)
                            _new_rows = []
                            for _r in _opt_rows:
                                _is_ccp = _r["단계명"] in _ccp_steps_set
                                if _is_ccp:
                                    # CCP 단계: Q1=아니오, Q2=예, Q3=예 → CCP
                                    _row = {c: "" for c in DEC_COLS}
                                    _row.update({
                                        "단계명": _r["단계명"], "위해유형": _r["위해유형"], "위해요소": _r["위해요소"],
                                        "Q1_답변": "아니오", "Q2_답변": "예", "Q21_답변": "",
                                        "Q3_답변": "예", "Q4_답변": "", "Q5_답변": "",
                                        "CCP결정": "CCP"
                                    })
                                else:
                                    # 일반 단계: Q1=아니오, Q2=예, Q3=아니오, Q4=예, Q5=예 → CP
                                    _row = {c: "" for c in DEC_COLS}
                                    _row.update({
                                        "단계명": _r["단계명"], "위해유형": _r["위해유형"], "위해요소": _r["위해요소"],
                                        "Q1_답변": "아니오", "Q2_답변": "예", "Q21_답변": "",
                                        "Q3_답변": "아니오", "Q4_답변": "예", "Q5_답변": "예",
                                        "CCP결정": "CP"
                                    })
                                _new_rows.append(_row)
                            df_opt = pd.DataFrame(_new_rows, columns=DEC_COLS)
                            save_ccp_decision(sel_cat_id, df_opt)
                            _ccp_cnt = sum(1 for r in _new_rows if r["CCP결정"] == "CCP")
                            _cp_cnt  = sum(1 for r in _new_rows if r["CCP결정"] == "CP")
                            st.success(f"✅ 스마트 최적화 완료! 총 {len(_new_rows)}항목 (CCP: {_ccp_cnt}개, CP: {_cp_cnt}개)")
                            st.rerun()
...
<tbody>"""
  ...
  <td style="{TD_CCP}background:{ccp_bg};color:{ccp_fc};font-weight:bold;">{ccp_r}</td>
            ...
            if st.session_state.ccp_mode == "list" and not df_dec.empty:
                excel_data = export_ccp_excel(df_dec, sel_cat_name, step_no=step_no)
                st.download_button(
    ...
    elif sub_menu == "원부자재 CCP 결정도":
        st.markdown('<div class="section-title">원부자재 CCP 결정도</div>', unsafe_allow_html=True)

        _df_rm_haz_base = load_rm_hazard().fillna("") if os.path.exists(RM_HAZARD_FILE) else pd.DataFrame()
        if _df_rm_haz_base.empty:
            st.info("원부자재 위해요소분석 데이터가 없습니다. '원부자재 위해요소분석' 메뉴에서 먼저 분석을 완료하세요.")
        else:
            _rm_haz_unique = (_df_rm_haz_base[["원자재명","원자재코드"]].drop_duplicates(subset=["원자재명"]).dropna(subset=["원자재명"]))
            _rm_haz_unique = _rm_haz_unique[_rm_haz_unique["원자재명"].astype(str).str.strip() != ""]
            _rm_pairs = [(str(r["원자재명"]).strip(), str(r["원자재코드"]).strip()) for _, r in _rm_haz_unique.iterrows()]

            if not _rm_pairs:
                st.info("위해요소분석에 등록된 원·부자재가 없습니다.")
            else:
                ANS3 = ["", "예", "아니오", "해당없음"]
                _rm_type_label = {"B": "B (생물학)", "C": "C (화학)", "P": "P (물리)"}
                TH_CCP2 = "background:#D6E4F0;color:#1F4E79;font-weight:bold;padding:8px 6px;text-align:center;border:1px solid #b0c4d8;font-size:11px;"
                TH_Q2   = "background:#2E75B6;color:white;font-weight:bold;padding:8px 4px;text-align:center;border:1px solid #1a5a9e;font-size:10px;"
                TH_Q3_2 = "background:#1A3E6E;color:white;font-weight:bold;padding:8px 4px;text-align:center;border:1px solid #0d2a52;font-size:10px;"
                TD_CCP2 = "background:#fff;padding:7px 6px;border:1px solid #d0d7de;vertical-align:middle;text-align:center;font-size:11px;word-break:break-word;"
                QBG2    = {"예":"#E2EFDA","아니오":"#FCE4D6","해당없음":"#F5F5F5"}

                if "rm_ccp_mode"        not in st.session_state: st.session_state.rm_ccp_mode        = "list"
                if "rm_ccp_active_code" not in st.session_state: st.session_state.rm_ccp_active_code = ""
                if "rm_ccp_edit_idx"    not in st.session_state: st.session_state.rm_ccp_edit_idx    = None

                # ── 자동 동기화 (전체 원부자재) ─────────────────────────
                _any_synced = False
                for _rn_s, _rc_s in _rm_pairs:
                    _df_ds = get_rm_ccp_decision(_rc_s)
                    _sync_n = 0
                    try:
                        _sync_src = _df_rm_haz_base[
                            (_df_rm_haz_base["원자재코드"].astype(str).str.strip() == _rc_s.strip()) &
                            (_df_rm_haz_base["최종위해요소"].astype(str).str.strip().str.upper() == "Y")
                        ]
                        for _, _hr in _sync_src.iterrows():
                            _hn = str(_hr.get("원자재명","")).strip()
                            _ht = str(_hr.get("유형","")).strip()[:1]
                            _hh = str(_hr.get("위해요소","")).strip()
                            _ex = False
                            if not _df_ds.empty:
                                _mm = ((_df_ds["단계명"].astype(str).str.strip() == _hn) & (_df_ds["위해요소"].astype(str).str.strip() == _hh))
                                _ex = _mm.any()
                                if _ex and _ht:
                                    _wr = _mm & (_df_ds["위해유형"].astype(str).str.strip().str[:1] != _ht)
                                    if _wr.any():
                                        _df_ds.loc[_wr, "위해유형"] = _ht
                                        _sync_n += 1
                            if not _ex:
                                _df_ds = pd.concat([_df_ds, pd.DataFrame([{
                                    "단계명":_hn,"위해유형":_ht,"위해요소":_hh,
                                    "Q1_답변":"","Q1_비고":"","Q2_답변":"","Q2_비고":"",
                                    "Q21_답변":"","Q21_비고":"","Q3_답변":"","Q3_비고":"",
                                    "Q4_답변":"","Q4_비고":"","Q5_답변":"","Q5_비고":"",
                                    "CCP결정":"","CCP번호":"","비고":""
                                ...
                                _sync_n += 1
                        if _sync_n > 0:
                            save_rm_ccp_decision(_rc_s, _df_ds)
                            _any_synced = True
                    except Exception:
                        pass
                if _any_synced:
                ...
                # ── 전체 데이터 수집 ────────────────────────────────────
                _all_rows = []
                for _rn, _rc in _rm_pairs:
                    _safe_c = str(_rc).replace("/","_").replace("\\","_").replace(" ","_")
                    _df_d = get_rm_ccp_decision(_rc)
                    for _i, _r in _df_d.iterrows():
                        _all_rows.append((_rn, _rc, _safe_c, _i, _r))

                _cur_mode = st.session_state.rm_ccp_mode
                _active_c = st.session_state.rm_ccp_active_code

                with st.expander("📋 CCP 결정 질문 흐름 보기", expanded=False):
                    st.markdown("""
| 질문 | 내용 | 예 | 아니오 |
|------|------|----|--------|
| **질문1** | 선행요건이 있으며 잘 관리되고 있는가? | → CP | → 질문2 |
| **질문2** | 위해요소에 대한 조치방법이 있는가? | → 질문3 | → 질문2-1 |
| **질문2-1** | 안전성을 위한 관리가 필요한가? | → 질문2 재검토 | → CP |
| **질문3** | 위해요소를 제거하거나 허용수준까지 감소시킬 수 있는가? | **→ CCP** | → 질문4 |
| **질문4** | 위해요소의 오염이 허용수준을 초과하는가? | → 질문5 | → CP |
| **질문5** | 이후 공정에서 제거/감소시킬 수 있는가? | → CP | **→ CCP** |
                    """)
                ...
                if _cur_mode == "list":
                    ...
                    with _bc1:
                        if st.button("항목 추가", type="primary", use_container_width=True, key="rmccp_add_btn"):
                            st.session_state.rm_ccp_mode = "add"
                            st.session_state.rm_ccp_active_code = _rm_pairs[0][1] if _rm_pairs else ""
                            st.session_state.rm_ccp_edit_idx = None
                            st.rerun()
                    with _bc2:
                        if st.button("🚀 스마트 최적화 (전체)", type="secondary", use_container_width=True, key="rmccp_opt_all"):
                            _opt_changed = False
                            for _rn, _rc in _rm_pairs:
                                _dop = get_rm_ccp_decision(_rc)
                                if not _dop.empty:
                                    for _oi in _dop.index:
                                        for _ck,_cv in {"Q1_답변":"아니오","Q2_답변":"예","Q21_답변":"","Q3_답변":"아니오","Q4_답변":"예","Q5_답변":"예","CCP결정":"CP"}.items():
                                            _dop.at[_oi,_ck]=_cv
                                    save_rm_ccp_decision(_rc, _dop.reset_index(drop=True))
                                    _opt_changed = True
                            if _opt_changed:
                                st.success("✅ 전체 CP 최적화 완료"); st.rerun()
                            else:
                                st.warning("등록된 항목이 없습니다.")

                    if not _all_rows:
                        st.info("등록된 항목이 없습니다. '항목 추가' 버튼 또는 위해요소분석을 먼저 완료하세요.")
                    else:
                        _n = len(_all_rows); _spn = [0]*_n; _ii = 0
                        while _ii < _n:
                            _jj = _ii
                            while _jj < _n and _all_rows[_jj][0] == _all_rows[_ii][0]: _jj += 1
                            _spn[_ii] = _jj - _ii; _ii = _jj
                        _html = (
                            f"<style>.rmccp2{{width:100%;border-collapse:collapse;}}"
                            f".rmccp2 tr:hover td{{filter:brightness(0.97);}}</style>"
                            f"<table class='rmccp2'><thead><tr>"
                            f"<th style='{TH_CCP2}width:120px;'>원자재명</th>"
                            f"<th style='{TH_CCP2}width:40px;'>유형</th>"
                            f"<th style='{TH_CCP2}width:140px;'>위해요소</th>"
                            f"<th style='{TH_Q2}width:65px;'>질문1</th><th style='{TH_Q2}width:65px;'>질문2</th>"
                            f"<th style='{TH_Q2}width:65px;'>질문2-1</th><th style='{TH_Q3_2}width:65px;'>질문3</th>"
                            f"<th style='{TH_Q2}width:65px;'>질문4</th><th style='{TH_Q2}width:65px;'>질문5</th>"
                            f"<th style='{TH_CCP2}width:70px;'>CCP결정</th></tr></thead><tbody>"
                        )
                        for _ci, (_rn, _rc, _safe_c, _i, _r) in enumerate(_all_rows):
                            _q1=str(_r.get("Q1_답변","")); _q2=str(_r.get("Q2_답변",""))
                            _q21=str(_r.get("Q21_답변","")); _q3=str(_r.get("Q3_답변",""))
                            _q4=str(_r.get("Q4_답변","")); _q5=str(_r.get("Q5_답변",""))
                            _cr=str(_r.get("CCP결정","")); _ht=str(_r.get("위해유형","")); _hz=str(_r.get("위해요소",""))
                            _act=active_questions(_q1,_q2,_q21,_q3,_q4)
                            _cbg="#D32F2F" if _cr.startswith("CCP") else ("#2E7D32" if _cr=="CP" else ("#F57C00" if _cr else "#fff"))
                            _cfc="white" if _cr.startswith("CCP") or _cr=="CP" else "#888"
                            def _qt2(qk,qv,_act=_act):
                                if qk not in _act: return f"<td style='{TD_CCP2}background:#eee;color:#aaa;'>—</td>"
                                _qbg=QBG2.get(qv,"#fff")
                                return f"<td style='{TD_CCP2}background:{_qbg};'>{qv}</td>"
                            _std = f'<td rowspan="{_spn[_ci]}" style="{TD_CCP2}background:#f0f4f8;font-weight:bold;vertical-align:middle;">{_rn}</td>' if _spn[_ci] > 0 else ""
                            _html += (
                                f"<tr>{_std}<td style='{TD_CCP2}'>{_ht}</td>"
                                f"<td style='{TD_CCP2}text-align:left;'>{_hz}</td>"
                                f"{_qt2('Q1',_q1)}{_qt2('Q2',_q2)}{_qt2('Q21',_q21)}"
                                f"{_qt2('Q3',_q3)}{_qt2('Q4',_q4)}{_qt2('Q5',_q5)}"
                                f"<td style='{TD_CCP2}background:{_cbg};color:{_cfc};font-weight:bold;'>{_cr}</td></tr>"
                            )
                        _html += "</tbody></table>"
                        ...
                        st.caption("수정·삭제:")
                        for _ci, (_rn, _rc, _safe_c, _i, _r) in enumerate(_all_rows):
                            _ht2=str(_r.get("위해유형","")); _hz2=str(_r.get("위해요소",""))
                            _cc1,_cc2,_cc3=st.columns([5,0.7,0.7])
                            with _cc1: st.caption(f"**{_rn}** | **{_ht2}** — {_hz2}")
                            with _cc2:
                                if st.button("✏️",key=f"rmed_{_safe_c}_{_i}",use_container_width=True):
                                    st.session_state.rm_ccp_mode="edit"; st.session_state.rm_ccp_active_code=_rc; st.session_state.rm_ccp_edit_idx=_i; st.rerun()
                            with _cc3:
                                if st.button("🗑️",key=f"rmdl_{_safe_c}_{_i}",use_container_width=True):
                                    _dc=get_rm_ccp_decision(_rc)
                                    _mk=_dc.apply(lambda r:(str(r.get("단계명","")).strip()==_rn.strip())and(str(r.get("위해요소","")).strip()==_hz2.strip()),axis=1)
                                    if _mk.any(): save_rm_ccp_decision(_rc,_dc[~_mk].reset_index(drop=True)); st.success("✅ 삭제 완료"); st.rerun()
                                    else: st.error("삭제 대상을 찾을 수 없습니다.")
                        _df_all_exp = pd.concat([get_rm_ccp_decision(_rc) for _rn, _rc in _rm_pairs], ignore_index=True)
                        if not _df_all_exp.empty:
                            _xls = export_ccp_excel(_df_all_exp, "원부자재전체")
                            st.download_button(label="📥 원부자재 CCP결정도 전체 엑셀", data=_xls, file_name=f"Hollys_원부자재CCP_전체_{date.today().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="rmccp_xls_all")

                # ════════════════════════════════════════════════════════
                # 추가 / 수정 모드
                # ════════════════════════════════════════════════════════
                elif _cur_mode in ("add", "edit"):
                    _is_edit_rm = (_cur_mode == "edit")
                    _edit_idx_rm = st.session_state.rm_ccp_edit_idx
                    if not _is_edit_rm:
                        _rm_names = [rn for rn, rc in _rm_pairs]
                        _sel_rm_name = st.selectbox("원자재 선택", _rm_names, key="rmccp_sel_rm")
                        _sel_rm_code = next((rc for rn, rc in _rm_pairs if rn == _sel_rm_name), "")
                        st.session_state.rm_ccp_active_code = _sel_rm_code
                    else:
                        _sel_rm_code = _active_c
                        _sel_rm_name = next((rn for rn, rc in _rm_pairs if rc == _sel_rm_code), _sel_rm_code)
                    _safe_c = str(_sel_rm_code).replace("/","_").replace("\\","_").replace(" ","_")
                    _df_rmd = get_rm_ccp_decision(_sel_rm_code)
                    RM_PFX = f"rmccp_{_safe_c}_{_edit_idx_rm}"
                    _rm_init_key = f"{RM_PFX}_init"

                    def _gv_rm(col):
                        if _is_edit_rm and _edit_idx_rm is not None and not _df_rmd.empty:
                            try:
                                v = _df_rmd.iloc[_edit_idx_rm][col]
                                return str(v) if pd.notna(v) else ""
                            except Exception: pass
                        return ""

                    if _rm_init_key not in st.session_state:
                        st.session_state[f"{RM_PFX}_htype"]    = _gv_rm("위해유형")
                        st.session_state[f"{RM_PFX}_hz"]       = _gv_rm("위해요소")
                        st.session_state[f"{RM_PFX}_Q1"]       = _gv_rm("Q1_답변")  or ""
                        st.session_state[f"{RM_PFX}_Q2"]       = _gv_rm("Q2_답변")  or ""
                        st.session_state[f"{RM_PFX}_Q21"]      = _gv_rm("Q21_답변") or ""
                        st.session_state[f"{RM_PFX}_Q3"]       = _gv_rm("Q3_답변")  or ""
                        st.session_state[f"{RM_PFX}_Q4"]       = _gv_rm("Q4_답변")  or ""
                        st.session_state[f"{RM_PFX}_Q5"]       = _gv_rm("Q5_답변")  or ""
                        st.session_state[f"{RM_PFX}_Q1_note"]  = _gv_rm("Q1_비고")
                        st.session_state[f"{RM_PFX}_Q2_note"]  = _gv_rm("Q2_비고")
                        st.session_state[f"{RM_PFX}_Q21_note"] = _gv_rm("Q21_비고")
                        st.session_state[f"{RM_PFX}_Q3_note"]  = _gv_rm("Q3_비고")
                        st.session_state[f"{RM_PFX}_Q4_note"]  = _gv_rm("Q4_비고")
                        st.session_state[f"{RM_PFX}_Q5_note"]  = _gv_rm("Q5_비고")
                        st.session_state[f"{RM_PFX}_ccpno"]    = _gv_rm("CCP번호")
                        st.session_state[f"{RM_PFX}_bigo"]     = _gv_rm("비고")
                        st.session_state[_rm_init_key] = True

                    st.markdown(f"#### {'✏️ 항목 수정' if _is_edit_rm else '항목 추가'} — {_sel_rm_name}")
                    st.text_input("원자재명 (단계명)", value=_sel_rm_name, disabled=True, key=f"{RM_PFX}_nm_disp")

                    _rm_this_haz = _df_rm_haz_base[_df_rm_haz_base["원자재코드"].astype(str).str.strip() == _sel_rm_code.strip()]
                    _avail_codes = sorted(_rm_this_haz["유형"].astype(str).str.strip().str[:1].unique().tolist())
                    _avail_codes = [c for c in _avail_codes if c in _rm_type_label]
                    _avail_types = [""] + [_rm_type_label[c] for c in _avail_codes]
                    _cur_ht = st.session_state[f"{RM_PFX}_htype"]
                    _ht_idx = next((i for i,v in enumerate(_avail_types) if v.startswith(_cur_ht[:1])), 0) if _cur_ht else 0
                    _f_htype = st.selectbox("유형", _avail_types, index=_ht_idx, key=f"{RM_PFX}_htype_sel")
                    st.session_state[f"{RM_PFX}_htype"] = _f_htype

                    _ht_code = _f_htype[:1] if _f_htype else ""
                    _m2 = _df_rm_haz_base["원자재코드"].astype(str).str.strip() == _sel_rm_code.strip()
                    if _ht_code:
                        _m2 = _m2 & (_df_rm_haz_base["유형"].astype(str).str.strip().str.startswith(_ht_code))
                    _hz_list = _df_rm_haz_base[_m2 & (_df_rm_haz_base["최종위해요소"].astype(str).str.strip().str.upper() == "Y")]["위해요소"].tolist()

                    if _f_htype:
                        if _is_edit_rm:
                            st.info(f"수정 중인 위해요소: **{st.session_state[f'{RM_PFX}_hz']}**")
                            _f_hz = st.session_state[f"{RM_PFX}_hz"]
                        else:
                            if _hz_list:
                                _bdg = "".join([f"<span style='display:inline-block;background:#D6E8FF;color:#1A5FAD;border-radius:12px;padding:3px 10px;margin:2px 3px;font-size:12px;font-weight:bold;'>{h}</span>" for h in _hz_list])
                                st.markdown(f"<div style='background:#f0f6ff;border:1px solid #b0ccee;border-radius:6px;padding:10px 14px;margin:6px 0;'><div style='font-size:12px;color:#1A5FAD;font-weight:bold;margin-bottom:6px;'>🔵 최종위해요소 자동 도출 ({len(_hz_list)}건)</div>{_bdg}</div>", unsafe_allow_html=True)
                                _f_hz = "__AUTO__"
                            else:
                                st.warning("해당 유형의 최종위해요소가 없습니다.")
                                _f_hz = ""
                        st.session_state[f"{RM_PFX}_hz"] = _f_hz
                    else:
                        _f_hz = st.session_state[f"{RM_PFX}_hz"]

                    st.markdown("---")
                    st.markdown("#### 📋 질문 답변")
                    st.info("💡 원부자재 기본 경로: Q1=아니오, Q2=예, Q3=아니오, Q4=예, Q5=예 → **CP**")
                    _dc1, _dc2 = st.columns([3,1])
                    with _dc2:
                        if st.button("🔄 기본값 자동채우기", use_container_width=True, key=f"{RM_PFX}_autofill"):
                            st.session_state[f"{RM_PFX}_Q1"]="아니오"; st.session_state[f"{RM_PFX}_Q2"]="예"
                            st.session_state[f"{RM_PFX}_Q21"]=""; st.session_state[f"{RM_PFX}_Q3"]="아니오"
                            st.session_state[f"{RM_PFX}_Q4"]="예"; st.session_state[f"{RM_PFX}_Q5"]="예"
                            st.rerun()

                    _rq1=st.session_state[f"{RM_PFX}_Q1"]; _rq2=st.session_state[f"{RM_PFX}_Q2"]
                    _rq21=st.session_state[f"{RM_PFX}_Q21"]; _rq3=st.session_state[f"{RM_PFX}_Q3"]
                    _rq4=st.session_state[f"{RM_PFX}_Q4"]; _rq5=st.session_state[f"{RM_PFX}_Q5"]
                    _rm_act = active_questions(_rq1,_rq2,_rq21,_rq3,_rq4)

                    def _render_q_rm(qlbl, qtxt, ss_key, ss_note_key, active, hint=""):
                        _bc="#2E75B6" if active else "#ccc"; _bgc="#EBF3FB" if active else "#F5F5F5"; _fc="#000" if active else "#aaa"
                        _hh=f' <span style="color:#D32F2F;font-weight:bold;">{hint}</span>' if hint and active else ""
                        st.markdown(f"<div style='background:{_bgc};border-left:4px solid {_bc};padding:8px 12px;border-radius:4px;margin:6px 0;'><span style='font-size:12px;font-weight:bold;color:{_fc};'>{qlbl}</span><span style='font-size:11px;color:{_fc};'>&nbsp;{qtxt}</span>{_hh}</div>", unsafe_allow_html=True)
                        if active:
                            _qa,_qn=st.columns([1,3])
                            with _qa:
                                _ca=st.session_state.get(ss_key,""); _ia=ANS3.index(_ca) if _ca in ANS3 else 0
                                _a=st.radio("답변",ANS3[1:],index=max(0,_ia-1),horizontal=True,key=f"radio_{ss_key}",label_visibility="collapsed")
                                st.session_state[ss_key]=_a
                            with _qn:
                                _nt=st.text_input("비고/근거",value=st.session_state.get(ss_note_key,""),key=f"note_{ss_key}",placeholder="근거·출처 (선택)")
                                st.session_state[ss_note_key]=_nt
                            return _a
                        else:
                            st.markdown("<div style='padding:4px 12px;color:#aaa;font-size:11px;'>⏸ 건너뜁니다.</div>", unsafe_allow_html=True)
                            st.session_state[ss_key]=""; return ""

                    _rq1=_render_q_rm("질문 1","확인된 위해요소를 관리하기 위한 선행요건이 있으며 잘 관리되고 있는가?",f"{RM_PFX}_Q1",f"{RM_PFX}_Q1_note",True,"→ CP" if _rq1=="예" else "→ 질문2" if _rq1=="아니오" else "")
                    _rm_act=active_questions(_rq1,_rq2,_rq21,_rq3,_rq4)
                    _rq2=_render_q_rm("질문 2","모든 공정(단계)에서 확인된 위해요소에 대한 조치방법이 있는가?",f"{RM_PFX}_Q2",f"{RM_PFX}_Q2_note","Q2" in _rm_act,"→ 질문3" if _rq2=="예" else "→ 질문2-1" if _rq2=="아니오" else "")
                    _rm_act=active_questions(_rq1,_rq2,_rq21,_rq3,_rq4)
                    _rq21=_render_q_rm("질문 2-1","이 공정(단계)에서 안전성을 위한 관리가 필요한가?",f"{RM_PFX}_Q21",f"{RM_PFX}_Q21_note","Q21" in _rm_act,"→ 질문2 재검토" if _rq21=="예" else "→ CP" if _rq21=="아니오" else "")
                    _rm_act=active_questions(_rq1,_rq2,_rq21,_rq3,_rq4)
                    _rq3=_render_q_rm("질문 3","이 공정(단계)에서 발생가능성이 있는 위해요소를 제거하거나 허용수준까지 감소시킬 수 있는가?",f"{RM_PFX}_Q3",f"{RM_PFX}_Q3_note","Q3" in _rm_act,"🔴 → CCP!" if _rq3=="예" else "→ 질문4" if _rq3=="아니오" else "")
                    _rm_act=active_questions(_rq1,_rq2,_rq21,_rq3,_rq4)
                    _rq4=_render_q_rm("질문 4","확인된 위해요소의 오염이 허용수준을 초과하는가 또는 허용할 수 없는 수준으로 증가하는가?",f"{RM_PFX}_Q4",f"{RM_PFX}_Q4_note","Q4" in _rm_act,"→ 질문5" if _rq4=="예" else "→ CP" if _rq4=="아니오" else "")
                    _rm_act=active_questions(_rq1,_rq2,_rq21,_rq3,_rq4)
                    _rq5=_render_q_rm("질문 5","확인된 위해요소를 제거하거나 그 발생을 허용수준으로 감소시킬 수 있는 이후의 공정이 있는가?",f"{RM_PFX}_Q5",f"{RM_PFX}_Q5_note","Q5" in _rm_act,"→ CP" if _rq5=="예" else "🔴 → CCP!" if _rq5=="아니오" else "")

                    _auto_rm=calc_ccp(_rq1,_rq2,_rq21,_rq3,_rq4,_rq5)
                    if not _auto_rm and _rq1=="아니오" and _rq2=="예" and _rq3=="아니오" and _rq4=="예" and _rq5=="예": _auto_rm="CP"
                    _bcc="#D32F2F" if _auto_rm=="CCP" else ("#2E7D32" if _auto_rm=="CP" else "#F57C00")
                    st.markdown(f'<div style="margin:14px 0;padding:12px 20px;border-radius:10px;border:2px solid {_bcc};background:#fafafa;display:flex;align-items:center;"><span style="font-size:13px;font-weight:bold;">📊 자동 판정:</span><span style="background:{_bcc};color:white;border-radius:6px;padding:4px 20px;font-size:16px;font-weight:bold;margin-left:12px;">{_auto_rm if _auto_rm else "미결정 — 질문을 완성하세요"}</span></div>', unsafe_allow_html=True)

                    _fca,_fcb=st.columns(2)
                    with _fca:
                        _f_ccpno=st.text_input("CCP번호",value=st.session_state[f"{RM_PFX}_ccpno"],placeholder="CCP-1",key=f"{RM_PFX}_ccpno_inp"); st.session_state[f"{RM_PFX}_ccpno"]=_f_ccpno
                    with _fcb:
                        _f_bigo=st.text_input("비고",value=st.session_state[f"{RM_PFX}_bigo"],key=f"{RM_PFX}_bigo_inp"); st.session_state[f"{RM_PFX}_bigo"]=_f_bigo

                    _sb1,_sb2=st.columns(2)
                    with _sb1:
                        if st.button("저장",type="primary",use_container_width=True,key=f"{RM_PFX}_save"):
                            if not _is_edit_rm and _f_hz=="__AUTO__" and not _hz_list:
                                st.error("최종위해요소가 없습니다.")
                            elif not _is_edit_rm and not _f_hz:
                                st.error("유형을 선택하세요.")
                            else:
                                _hts=_f_htype[:1] if _f_htype else ""
                                _qd={"Q1_답변":st.session_state[f"{RM_PFX}_Q1"],"Q1_비고":st.session_state[f"{RM_PFX}_Q1_note"],"Q2_답변":st.session_state[f"{RM_PFX}_Q2"],"Q2_비고":st.session_state[f"{RM_PFX}_Q2_note"],"Q21_답변":st.session_state[f"{RM_PFX}_Q21"],"Q21_비고":st.session_state[f"{RM_PFX}_Q21_note"],"Q3_답변":st.session_state[f"{RM_PFX}_Q3"],"Q3_비고":st.session_state[f"{RM_PFX}_Q3_note"],"Q4_답변":st.session_state[f"{RM_PFX}_Q4"],"Q4_비고":st.session_state[f"{RM_PFX}_Q4_note"],"Q5_답변":st.session_state[f"{RM_PFX}_Q5"],"Q5_비고":st.session_state[f"{RM_PFX}_Q5_note"],"CCP결정":_auto_rm,"CCP번호":_f_ccpno,"비고":_f_bigo}
                                if _is_edit_rm and _edit_idx_rm is not None and not _df_rmd.empty:
                                    _nr={"단계명":_sel_rm_name,"위해유형":_hts,"위해요소":st.session_state[f"{RM_PFX}_hz"],**_qd}
                                    for _k,_v in _nr.items():
                                        if _k not in _df_rmd.columns: _df_rmd[_k]=""
                                        _df_rmd.at[_edit_idx_rm,_k]=_v
                                    save_rm_ccp_decision(_sel_rm_code,_df_rmd); _cnt=1
                                else:
                                    _sl=_hz_list if _f_hz=="__AUTO__" else [_f_hz]
                                    _df_rmd=pd.concat([_df_rmd,pd.DataFrame([{"단계명":_sel_rm_name,"위해유형":_hts,"위해요소":h,**_qd} for h in _sl])],ignore_index=True)
                                    save_rm_ccp_decision(_sel_rm_code,_df_rmd); _cnt=len(_sl)
                                for _k in list(st.session_state.keys()):
                                    if _k.startswith(RM_PFX): del st.session_state[_k]
                                st.session_state.rm_ccp_mode="list"; st.session_state.rm_ccp_active_code=""
                                st.success(f"✅ {_cnt}건 저장 완료!"); st.rerun()
                    with _sb2:
                        if st.button("취소",use_container_width=True,key=f"{RM_PFX}_cancel"):
                            for _k in list(st.session_state.keys()):
                                if _k.startswith(RM_PFX): del st.session_state[_k]
                            st.session_state.rm_ccp_mode="list"; st.session_state.rm_ccp_active_code=""; st.rerun()
...
# ──────────────────────────────────────────────────────────────────
        ...
        if cats_df.empty:
            ...
            if not os.path.exists(flow_file):
                ...
                df_flow2 = pd.read_csv(flow_file, encoding='utf-8-sig', dtype=str).fillna("")
                # 레인 필터: "공정" 레인 우선, 없으면 첫 번째 레인, 둘 다 없으면 전체
                if "lane" in df_flow2.columns:
                    _all_lanes = [l for l in df_flow2["lane"].str.strip().unique() if l]
                    if "공정" in _all_lanes:
                        mask_m = df_flow2["lane"].str.strip() == "공정"
                    elif _all_lanes:
                        mask_m = df_flow2["lane"].str.strip() == _all_lanes[0]
                    else:
                        mask_m = pd.Series([True]*len(df_flow2))
                elif "유형" in df_flow2.columns:
                    mask_m = df_flow2["유형"].str.strip().isin(["공정","검사","판단","보관","출하"])
                else:
                    mask_m = pd.Series([True]*len(df_flow2))
                ...
                # ── 🧠 스마트 위해요소 적용 ──────────────────────────────
                def _load_smart_cfg_h():
                    if os.path.exists("smart_hazard_config.csv"):
                        try: return pd.read_csv("smart_hazard_config.csv", encoding="utf-8-sig", dtype=str).fillna("")
                        except: pass
                    return pd.DataFrame(columns=["적용대상","대상키","대상명","유형","위해요소"])

                def _load_sev_h():
                    if os.path.exists(SEVERITY_FILE):
                        try:
                            df = pd.read_csv(SEVERITY_FILE, encoding="utf-8-sig", dtype=str).fillna("")
                            df.columns = ["유형","위해요소","심각성"]
                            return df
                        except: pass
                    return pd.DataFrame(columns=["유형","위해요소","심각성"])

                _cfg_h = _load_smart_cfg_h()
                _sev_h = _load_sev_h()
                _mask_h = (_cfg_h["적용대상"]=="공정") & (_cfg_h["대상키"].astype(str)==str(sel_cat_id))
                _cfg_cat = _cfg_h[_mask_h]

                if not _cfg_cat.empty:
                    _smart_count = len(_cfg_cat)
                    st.markdown(
                        f'<div style="background:#EBF3FB;border-left:4px solid #2E75B6;border-radius:6px;padding:10px 16px;margin:8px 0;">'
                        f'🧠 <b>스마트 위해요소 설정</b>: 현재 <b>{_smart_count}개</b> 위해요소가 설정되어 있습니다.'
                        f'</div>', unsafe_allow_html=True)
                    if st.button("🧠 스마트 위해요소분석 적용", type="primary", use_container_width=True, key=f"smart_h_apply_{sel_cat_id}"):
                        # 해당 카테고리 기존 데이터 전체 초기화 후 재적용
                        df_haz = df_haz[df_haz["카테고리"].astype(str).str.strip() != sel_cat_name].copy()
                        added_s = 0
                        for _, cfg_row in _cfg_cat.iterrows():
                            hz_t  = cfg_row["유형"]
                            hz_nm = cfg_row["위해요소"]
                            sev_match = _sev_h[(_sev_h["유형"]==hz_t) & (_sev_h["위해요소"]==hz_nm)]
                            sev_val = sev_match.iloc[0]["심각성"] if not sev_match.empty else ""
                            for ap in proc_list:
                                try:
                                    sv_s = int(sev_val) if sev_val else ""
                                    pb_s = 1
                                    total_s = sv_s * pb_s if isinstance(sv_s, int) else ""
                                    final_s = "Y" if isinstance(total_s, int) and total_s >= 6 else "N"
                                except: sv_s, pb_s, total_s, final_s = "", 1, "", "N"
                                new_h = {"카테고리": sel_cat_name, "No": "", "공정명": ap,
                                         "유형": hz_t, "위해요소": hz_nm,
                                         "발생원인": "", "심각성": str(sv_s),
                                         "발생가능성": str(pb_s), "종합평가": str(total_s),
                                         "최종위해요소": final_s, "예방조치및관리방법": ""}
                                df_haz = pd.concat([df_haz, pd.DataFrame([new_h])], ignore_index=True)
                                added_s += 1
                        df_haz.to_csv(HAZARD_FILE, index=False, encoding="utf-8-sig")
                        log_history("스마트 위해요소 적용(초기화)", sel_cat_name, f"{added_s}건 재적용")
                        st.success(f"✅ 기존 데이터를 초기화하고 {added_s}건을 새로 적용했습니다!")
                        st.rerun()

                # ── 🗑️ CCP 이후 위해요소 자동 정리 ─────────────────────
                _ccp_del_file_h = "ccp_after_delete_config.csv"
                if os.path.exists(_ccp_del_file_h):
                    _df_ccp_del_h = pd.read_csv(_ccp_del_file_h, encoding="utf-8-sig", dtype=str).fillna("")
                    if not _df_ccp_del_h.empty:
                        # 흐름도에 CCP유형이 설정된 CCP 단계가 있는지 확인
                        _df_flow_ck = pd.read_csv(flow_file, encoding='utf-8-sig', dtype=str).fillna("")
                        if "CCP유형" not in _df_flow_ck.columns: _df_flow_ck["CCP유형"] = ""
                        if "CCP여부" not in _df_flow_ck.columns: _df_flow_ck["CCP여부"] = "N"
                        _ccp_typed = _df_flow_ck[
                            (_df_flow_ck["CCP여부"].str.upper()=="Y") &
                            (_df_flow_ck["CCP유형"].str.strip() != "")
                        ]
                        if not _ccp_typed.empty:
                            st.markdown(
                                f'<div style="background:#FFF3E0;border-left:4px solid #E65100;border-radius:6px;padding:10px 16px;margin:8px 0;">'
                                f'🗑️ <b>CCP 이후 자동 정리</b>: 공정 흐름도에 CCP 유형이 설정된 단계가 <b>{len(_ccp_typed)}개</b> 있습니다. '
                                f'해당 CCP 이후 공정의 제어된 위해요소를 자동 삭제합니다.</div>',
                                unsafe_allow_html=True
                            )
                            if st.button("🗑️ CCP 이후 위해요소 자동 정리", use_container_width=True, key=f"ccp_after_del_{sel_cat_id}"):
                                _df_flow_del = pd.read_csv(flow_file, encoding='utf-8-sig', dtype=str).fillna("")
                                if "CCP유형" not in _df_flow_del.columns: _df_flow_del["CCP유형"] = ""
                                if "lane" not in _df_flow_del.columns: _df_flow_del["lane"] = ""
                                _df_flow_del["_no"] = pd.to_numeric(_df_flow_del["순서"], errors="coerce").fillna(9999)
                                _ccp_steps_del = _df_flow_del[
                                    (_df_flow_del["CCP여부"].str.upper()=="Y") &
                                    (_df_flow_del["CCP유형"].str.strip() != "")
                                ].sort_values("_no")
                                deleted_cnt = 0
                                for _, ccp_r in _ccp_steps_del.iterrows():
                                    _ccp_no  = ccp_r["_no"]
                                    _ccp_lane = str(ccp_r.get("lane","")).strip()
                                    _ccp_types = [t.strip() for t in str(ccp_r["CCP유형"]).split(",") if t.strip()]
                                    # 같은 레인의 이후 단계 단계명 목록
                                    _after_mask = _df_flow_del["_no"] > _ccp_no
                                    if _ccp_lane:
                                        _after_mask = _after_mask & (_df_flow_del["lane"].str.strip()==_ccp_lane)
                                    _after_steps = _df_flow_del[_after_mask]["단계명"].str.strip().tolist()
                                    for _t in _ccp_types:
                                        # B1, P1 등 숫자 붙은 유형 → 위해요소분석 유형 컬럼은 첫 글자(B/C/P)로 저장됨
                                        _base_t = _t[0].upper() if _t else _t
                                        _del_mask_h = (_df_ccp_del_h["CCP유형"]==_t)
                                        if "카테고리키" in _df_ccp_del_h.columns:
                                            _del_mask_h = _del_mask_h & (_df_ccp_del_h["카테고리키"].astype(str)==str(sel_cat_id))
                                        _del_list = _df_ccp_del_h[_del_mask_h]["위해요소"].str.strip().tolist()
                                        for _hz in _del_list:
                                            _mask_d = (
                                                (df_haz["카테고리"]==sel_cat_name) &
                                                (df_haz["공정명"].str.strip().isin(_after_steps)) &
                                                (df_haz["유형"].str.strip()==_base_t) &
                                                (df_haz["위해요소"].str.strip()==_hz)
                                            )
                                            deleted_cnt += int(_mask_d.sum())
                                            df_haz = df_haz[~_mask_d]
                                df_haz.to_csv(HAZARD_FILE, index=False, encoding="utf-8-sig")
                                log_history("CCP 이후 위해요소 정리", sel_cat_name, f"{deleted_cnt}건 삭제")
                                if deleted_cnt > 0:
                                    st.success(f"✅ {deleted_cnt}건이 삭제되었습니다!")
                                else:
                                    st.info("삭제할 항목이 없습니다. (미등록이거나 이미 정리됨)")
                                st.rerun()
                    ...
                    if cur_proc and cur_type:
                        ...
                        if not all_hz_display:
                            ...
                            if st.button("저장", type="primary", use_container_width=True, key=f"{PKX}_save"):
                                if not selected_hz and not st.session_state.get(f"{PKX}_confirm_empty"):
                                    st.session_state[f"{PKX}_confirm_empty"] = True
                                    st.warning("선택된 위해요소가 없습니다. 저장하면 해당 공정/유형의 모든 항목이 삭제됩니다. 다시 한 번 저장 버튼을 클릭하면 삭제됩니다.")
                                else:
                                    st.session_state.pop(f"{PKX}_confirm_empty", None)
                                    ...
                                    if new_rows:
                                        ...
                                        st.success("해당 공정/유형의 위해요소 항목이 모두 삭제됐습니다.")
                                    st.rerun()
                    else:
...
# ──────────────────────────────────────────────────────────────────
        ...
        if df_rm_list.empty:
            ...
            # ── 🧠 스마트 위해요소 적용 (원부자재) ────────────────────
            SMART_LIB_FILE_R = "smart_hazard_library.csv"
            SMART_CFG_FILE_R = "smart_hazard_config.csv"

            def _load_lib_r():
                if os.path.exists(SMART_LIB_FILE_R):
                    try: return pd.read_csv(SMART_LIB_FILE_R, encoding="utf-8-sig", dtype=str).fillna("")
                    except: pass
                return pd.DataFrame(columns=["유형","위해요소","심각성","발생원인","예방조치및관리방법"])

            def _load_cfg_r():
                if os.path.exists(SMART_CFG_FILE_R):
                    try: return pd.read_csv(SMART_CFG_FILE_R, encoding="utf-8-sig", dtype=str).fillna("")
                    except: pass
                return pd.DataFrame(columns=["적용대상","대상키","대상명","유형","위해요소"])

            _lib_r2 = _load_lib_r()
            _cfg_r2 = _load_cfg_r()
            # Pick RM for smart apply
            _rm_smart_disp = rm_display
            _rm_smart_sel = st.selectbox("🧠 스마트 적용할 원·부자재 선택", _rm_smart_disp, key="smart_r_rm_sel") if _rm_smart_disp else None
            if _rm_smart_sel and _rm_smart_sel in _rm_smart_disp:
                _rm_smart_idx = _rm_smart_disp.index(_rm_smart_sel)
                rm_sel_code = rm_codes[_rm_smart_idx]
                rm_sel_name = rm_names[_rm_smart_idx]
            elif rm_codes:
                rm_sel_code = rm_codes[0]
                rm_sel_name = rm_names[0]
            else:
                rm_sel_code = ""
                rm_sel_name = ""

            # 선택된 원부자재의 유형 조회 (whitespace 제거 후 비교)
            _rm_row_for_type = df_rm_list[df_rm_list["원자재코드"].str.strip()==str(rm_sel_code).strip()]
            _rm_sel_유형 = str(_rm_row_for_type.iloc[0]["유형"]).strip() if not _rm_row_for_type.empty else ""
            # 개별 코드 config + 유형별 config 합산 (strip 비교로 인코딩 문제 방지)
            _mask_r2_code = (
                (_cfg_r2["적용대상"].str.strip()=="원부자재") &
                (_cfg_r2["대상키"].str.strip()==str(rm_sel_code).strip())
            )
            if _rm_sel_유형:
                _mask_r2_type = (
                    (_cfg_r2["적용대상"].str.strip()=="원부자재유형") &
                    (_cfg_r2["대상키"].str.strip()==_rm_sel_유형)
                )
            else:
                _mask_r2_type = pd.Series([False]*len(_cfg_r2), index=_cfg_r2.index)
            _cfg_rm2 = _cfg_r2[_mask_r2_code | _mask_r2_type].drop_duplicates(subset=["유형","위해요소"])

            if not _cfg_rm2.empty:
                _smart_rm_count = len(_cfg_rm2)
                _유형_label = f" (유형: {_rm_sel_유형})" if _rm_sel_유형 else ""
                st.markdown(
                    f'<div style="background:#E3F2FD;border:1px solid #90CAF9;border-radius:8px;padding:10px 14px;margin-bottom:8px;">'
                    f'🧠 <b>스마트 위해요소 설정</b>{_유형_label}: 현재 <b>{_smart_rm_count}개</b> 위해요소 설정됨. '
                    f'적용 시 기존 항목을 초기화하고 재적용합니다.</div>',
                    unsafe_allow_html=True
                )
                if st.button("🧠 스마트 위해요소분석 적용", type="primary",
                             use_container_width=True, key=f"smart_r_apply_{rm_sel_code}"):
                    # 이미 로드된 df_sev_rm 사용 (재파싱 불필요)
                    _df_sev_r2 = df_sev_rm.copy()
                    if "유형" not in _df_sev_r2.columns or "위해요소" not in _df_sev_r2.columns:
                        _df_sev_r2 = pd.DataFrame(columns=["유형","위해요소","심각성"])
                    _rm_row_match2 = df_rm_list[df_rm_list["원자재코드"].str.strip()==str(rm_sel_code).strip()]
                    _rm_type_code2 = str(_rm_row_match2.iloc[0]["유형"]).strip() if not _rm_row_match2.empty else ""
                    # 해당 RM 기존 데이터 초기화 후 재적용
                    df_rm_haz = df_rm_haz[df_rm_haz["원자재코드"].astype(str).str.strip() != str(rm_sel_code).strip()].copy()
                    added_rs = 0
                    for _, cfg_rr in _cfg_rm2.iterrows():
                        hz_t  = str(cfg_rr["유형"]).strip()
                        hz_nm = str(cfg_rr["위해요소"]).strip()
                        sev_match = _df_sev_r2[
                            (_df_sev_r2["유형"].astype(str).str.strip()==hz_t) &
                            (_df_sev_r2["위해요소"].astype(str).str.strip()==hz_nm)
                        ]
                        sev_val = str(sev_match.iloc[0]["심각성"]).strip() if not sev_match.empty else ""
                        try:
                            sv_r2 = int(sev_val) if sev_val else ""
                            pb_r2 = 1
                            total_r2 = sv_r2 * pb_r2 if isinstance(sv_r2, int) else ""
                            final_r2 = "Y" if isinstance(total_r2, int) and total_r2 >= 6 else "N"
                        except: sv_r2, pb_r2, total_r2, final_r2 = "", 1, "", "N"
                        new_hr2 = {"원자재명":rm_sel_name,"원자재코드":rm_sel_code,
                            "유형코드":_rm_type_code2,
                            "No":"","유형":hz_t,"위해요소":hz_nm,
                            "발생원인":"","심각성":str(sv_r2),
                            "발생가능성":str(pb_r2),"종합평가":str(total_r2),
                            "최종위해요소":final_r2,"예방조치및관리방법":""}
                        df_rm_haz = pd.concat([df_rm_haz, pd.DataFrame([new_hr2])], ignore_index=True)
                        added_rs += 1
                    df_rm_haz.to_csv(_RM_HAZ_FILE, index=False, encoding="utf-8-sig")
                    log_history("스마트 위해요소 적용(초기화)", f"원부자재: {rm_sel_name}", f"{added_rs}건 재적용")
                    st.success(f"✅ 기존 데이터를 초기화하고 {added_rs}건을 새로 적용했습니다!")
                    st.rerun()
    ...
    # 스마트 위해요소 설정
    # ════════════════════════════════════════════════════════
    elif sub_menu == "스마트 위해요소 설정":
        st.markdown('<div class="section-title">🧠 스마트 위해요소 설정</div>', unsafe_allow_html=True)
        st.write("위해요소 라이브러리를 관리하고, 카테고리 및 원·부자재별로 적용할 위해요소를 설정합니다.")

        SMART_LIB_FILE = "smart_hazard_library.csv"
        SMART_CFG_FILE = "smart_hazard_config.csv"
        _LIB_COLS = ["유형", "위해요소", "심각성", "발생원인", "예방조치및관리방법"]
        _CFG_COLS = ["적용대상", "대상키", "대상명", "유형", "위해요소"]

        # Default library data
        _DEFAULT_LIB = [
            # B type
            ("B","장출혈성대장균","3","오염된 원료·분변 오염","가열처리, 입고검사"),
            ("B","클로스트리디움퍼프린젠스","1","토양·분변 오염","가열처리, 냉각관리"),
            ("B","살모넬라spp.","2","분변 오염, 오염된 원료","가열처리, 교차오염 방지"),
            ("B","리스테리아모노사이토제네스","3","토양·수분·저온환경","가열처리, 온도관리"),
            ("B","황색포도상구균","1","작업자 손 오염","개인위생, 가열처리"),
            ("B","바실러스세레우스","1","토양·분진 오염","가열처리, 온도관리"),
            ("B","대장균군","2","분변 오염","가열처리, 세척소독"),
            ("B","엔테로사카자키","2","건조식품 오염","위생관리 강화"),
            ("B","클로스트리디움보툴리눔","3","토양·혐기조건","가열처리, pH 관리"),
            ("B","총대장균군","2","분변 오염, 용수 오염","가열처리, 용수관리"),
            ("B","진균","2","수분·온도 조건","온·습도 관리"),
            ("B","대장균","2","분변 오염","가열처리, 교차오염 방지"),
            ("B","분원성대장균군","2","분변 오염","가열처리, 용수관리"),
            ("B","장염비브리오","2","해산물 오염","가열처리, 냉장관리"),
            ("B","여시니아엔테로콜리티카","2","냉장 식품 오염","가열처리, 냉장관리"),
            # C type
            ("C","잔류농약","3","원료 재배 시 농약 사용","입고검사, 공인시험성적서 확인"),
            ("C","중금속","3","토양·용수 오염","원산지 확인, 입고검사"),
            ("C","마이코톡신/곰팡이독소","3","보관 중 곰팡이 발생","온·습도 관리, 입고검사"),
            ("C","알레르기유발물질","2","교차 오염, 미표시","알레르기 관리 기준 준수"),
            ("C","식품첨가물","2","과량 첨가","사용기준 준수"),
            ("C","세척소독제잔류","2","세척 불충분","헹굼 기준 준수"),
            # P type
            ("P","금속이물","3","설비 파손, 원료 혼입","금속검출기, 입고검사"),
            ("P","유리이물","3","용기·조명 파손","유리관리 기준 준수"),
            ("P","플라스틱이물","2","포장재 파손, 원료 혼입","육안검사, 이물관리"),
            ("P","목재이물","2","팔레트·원료 혼입","입고검사, 이물관리"),
            ("P","돌·모래","2","원료 혼입","선별공정, 입고검사"),
        ]

        def load_smart_lib():
            if os.path.exists(SMART_LIB_FILE):
                try:
                    df = pd.read_csv(SMART_LIB_FILE, encoding="utf-8-sig", dtype=str).fillna("")
                    for c in _LIB_COLS:
                        if c not in df.columns: df[c] = ""
                    return df[_LIB_COLS]
                except Exception: pass
            # Create default
            df = pd.DataFrame(_DEFAULT_LIB, columns=_LIB_COLS)
            df.to_csv(SMART_LIB_FILE, index=False, encoding="utf-8-sig")
            return df

        def load_smart_cfg():
            if os.path.exists(SMART_CFG_FILE):
                try:
                    df = pd.read_csv(SMART_CFG_FILE, encoding="utf-8-sig", dtype=str).fillna("")
                    for c in _CFG_COLS:
                        if c not in df.columns: df[c] = ""
                    return df[_CFG_COLS]
                except Exception: pass
            return pd.DataFrame(columns=_CFG_COLS)

        def save_smart_lib(df):
            df.to_csv(SMART_LIB_FILE, index=False, encoding="utf-8-sig")

        def save_smart_cfg(df):
            df.to_csv(SMART_CFG_FILE, index=False, encoding="utf-8-sig")

        def sev_color(s):
            try:
                sv = int(s)
                if sv >= 3: return "#D32F2F"
                if sv == 2: return "#FF9800"
                return "#4CAF50"
            except: return "#888"

        df_lib = load_smart_lib()
        df_cfg = load_smart_cfg()

        # 심각성 설정에서 위해요소 목록 로드 (Tab2/Tab3 체크박스 소스)
        _SEV_FILE_SM = "severity_settings.csv"
        if os.path.exists(_SEV_FILE_SM):
            try:
                df_sev_sm = pd.read_csv(_SEV_FILE_SM, encoding="utf-8-sig", dtype=str).fillna("")
                df_sev_sm.columns = ["유형","위해요소","심각성"]
            except Exception:
                df_sev_sm = df_lib
        else:
            df_sev_sm = df_lib

        _cats_df_sm = pd.read_csv("haccp_flow_categories.csv", encoding="utf-8-sig") \
            if os.path.exists("haccp_flow_categories.csv") else pd.DataFrame()
        _rm_list_sm = pd.read_csv(RM_FILE, dtype=str).fillna("") \
            if os.path.exists(RM_FILE) else pd.DataFrame()

        CCP_DEL_FILE = "ccp_after_delete_config.csv"
        _CCP_DEL_COLS = ["카테고리키", "카테고리명", "CCP유형", "위해요소"]

        def load_ccp_del_cfg():
            if os.path.exists(CCP_DEL_FILE):
                try:
                    df = pd.read_csv(CCP_DEL_FILE, encoding="utf-8-sig", dtype=str).fillna("")
                    # 구버전(카테고리키 없음) → 새 포맷으로 마이그레이션
                    if "카테고리키" not in df.columns:
                        df.to_csv(CCP_DEL_FILE + ".bak", index=False, encoding="utf-8-sig")
                        df = pd.DataFrame(columns=_CCP_DEL_COLS)
                        df.to_csv(CCP_DEL_FILE, index=False, encoding="utf-8-sig")
                    for c in _CCP_DEL_COLS:
                        if c not in df.columns: df[c] = ""
                    return df[_CCP_DEL_COLS]
                except Exception: pass
            df = pd.DataFrame(columns=_CCP_DEL_COLS)
            df.to_csv(CCP_DEL_FILE, index=False, encoding="utf-8-sig")
            return df

        tab_lib, tab_proc_cfg, tab_rm_cfg, tab_ccp_del = st.tabs(["📚 위해요소 라이브러리", "⚙️ 카테고리별 설정", "📦 원부자재별 설정", "🗑️ CCP 이후 삭제 설정"])

        # ── Tab1: Library management ──────────────────────────────
        with tab_lib:
            st.markdown("#### 📚 위해요소 라이브러리 관리")
            st.caption("공통으로 사용할 표준 위해요소 목록입니다. 심각성·발생원인·예방조치를 미리 설정합니다.")

            TH_L = "background:#1F4E79;color:#fff;font-weight:bold;padding:7px 8px;text-align:center;border:1px solid #1a3e6a;font-size:11px;"
            TD_L = "background:#fff;padding:6px 8px;border:1px solid #d0d7de;font-size:11px;vertical-align:middle;"

            for hz_type in ["B", "C", "P"]:
                type_label = {"B": "🦠 생물학적 (B)", "C": "⚗️ 화학적 (C)", "P": "🔩 물리적 (P)"}[hz_type]
                df_t = df_lib[df_lib["유형"] == hz_type].reset_index(drop=True)
                with st.expander(f"{type_label} — {len(df_t)}종", expanded=(hz_type=="B")):
                    if not df_t.empty:
                        html = f'<table style="width:100%;border-collapse:collapse;margin-bottom:8px;"><tr>'
                        for h in ["위해요소","심각성","발생원인","예방조치및관리방법"]:
                            html += f'<th style="{TH_L}">{h}</th>'
                        html += "</tr>"
                        for _, row in df_t.iterrows():
                            sc = sev_color(row["심각성"])
                            sev_badge = f'<span style="background:{sc};color:#fff;border-radius:4px;padding:2px 8px;font-weight:bold;">{row["심각성"]}</span>'
                            html += f'<tr><td style="{TD_L}"><b>{row["위해요소"]}</b></td><td style="{TD_L};text-align:center;">{sev_badge}</td><td style="{TD_L}">{row["발생원인"]}</td><td style="{TD_L}">{row["예방조치및관리방법"]}</td></tr>'
                        html += "</table>"
                        st.markdown(html, unsafe_allow_html=True)
                    else:
                    ...
                    with lc1:
                        new_hz_name = st.text_input("위해요소명", key=f"lib_hz_{hz_type}", placeholder="예: 캄필로박터 제주니")
                    with lc2:
                        new_hz_sev = st.selectbox("심각성", ["1","2","3","4","5"], key=f"lib_sev_{hz_type}")
                    lc3, lc4 = st.columns(2)
                    with lc3:
                        new_hz_cause = st.text_input("발생원인", key=f"lib_cause_{hz_type}")
                    with lc4:
                        new_hz_prev = st.text_input("예방조치", key=f"lib_prev_{hz_type}")
                    if st.button(f"{type_label} 항목 추가", key=f"lib_add_{hz_type}", type="primary"):
                        if new_hz_name.strip():
                            dup = ((df_lib["유형"]==hz_type) & (df_lib["위해요소"]==new_hz_name.strip())).any()
                            if dup:
                                st.error("이미 라이브러리에 존재하는 항목입니다.")
                            else:
                                new_row = pd.DataFrame([{"유형":hz_type,"위해요소":new_hz_name.strip(),
                                    "심각성":new_hz_sev,"발생원인":new_hz_cause.strip(),
                                    "예방조치및관리방법":new_hz_prev.strip()}])
                                df_lib = pd.concat([df_lib, new_row], ignore_index=True)
                                save_smart_lib(df_lib)
                                st.success(f"✅ [{new_hz_name.strip()}] 추가 완료!")
                                st.rerun()
                        else:
                            st.error("위해요소명을 입력하세요.")

                    if not df_t.empty:
                        del_opts_l = df_t["위해요소"].tolist()
                        del_sel_l = st.multiselect("삭제할 항목", del_opts_l, key=f"lib_del_{hz_type}")
                        if del_sel_l and st.button("선택 삭제", key=f"lib_del_btn_{hz_type}", type="secondary"):
                            df_lib = df_lib[~((df_lib["유형"]==hz_type) & (df_lib["위해요소"].isin(del_sel_l)))].reset_index(drop=True)
                            save_smart_lib(df_lib)
                            st.success("삭제 완료!")
                            st.rerun()

        # ── Tab2: Per-category configuration ─────────────────────
        with tab_proc_cfg:
            st.markdown("#### ⚙️ 카테고리별 적용 위해요소 설정")
            st.caption("카테고리를 선택하고, 스마트 적용 시 사용할 위해요소에 체크하세요.")

            if _cats_df_sm.empty or "cat_name" not in _cats_df_sm.columns:
                st.warning("공정 흐름도 카테고리가 없습니다.")
            else:
                pc_cat_names = _cats_df_sm["cat_name"].tolist()
                pc_cat_ids   = _cats_df_sm["cat_id"].tolist()
                pc_sel_cat   = st.selectbox("카테고리 선택", pc_cat_names, key="smart_cfg_cat")
                pc_cat_idx   = pc_cat_names.index(pc_sel_cat)
                pc_cat_id    = pc_cat_ids[pc_cat_idx]

                # Currently enabled hazards for this category
                mask_pc = (df_cfg["적용대상"] == "공정") & (df_cfg["대상키"] == str(pc_cat_id))
                enabled_pc = set(zip(df_cfg[mask_pc]["유형"], df_cfg[mask_pc]["위해요소"]))

                st.markdown(f"**현재 [{pc_sel_cat}] 설정: {len(enabled_pc)}개 항목 활성화**")

                new_enabled_pc = set()
                for hz_type in ["B", "C", "P"]:
                    type_label2 = {"B":"🦠 생물학적","C":"⚗️ 화학적","P":"🔩 물리적"}[hz_type]
                    df_lt = df_sev_sm[df_sev_sm["유형"] == hz_type].reset_index(drop=True)
                    if df_lt.empty: continue
                    st.markdown(f"**{type_label2}**")

                    all_checked = all((hz_type, r["위해요소"]) in enabled_pc for _, r in df_lt.iterrows())
                    toggle_all = st.checkbox(f"전체 선택 ({type_label2})", value=all_checked, key=f"pc_all_{hz_type}_{pc_cat_id}")

                    for ri, row in df_lt.iterrows():
                        key = (hz_type, row["위해요소"])
                        default_val = toggle_all or key in enabled_pc
                        sev_str = f"[심각성 {row['심각성']}]"
                        checked = st.checkbox(
                            f"{row['위해요소']}  {sev_str}",
                            value=default_val,
                            key=f"pc_chk_{hz_type}_{ri}_{pc_cat_id}"
                        )
                        if checked:
                            new_enabled_pc.add(key)

                if st.button("💾 설정 저장", type="primary", use_container_width=True, key="pc_cfg_save"):
                    # Remove old config for this cat, add new
                    df_cfg_other = df_cfg[~((df_cfg["적용대상"]=="공정") & (df_cfg["대상키"]==str(pc_cat_id)))]
                    new_rows = [{"적용대상":"공정","대상키":str(pc_cat_id),"대상명":pc_sel_cat,"유형":t,"위해요소":h}
                                for (t,h) in new_enabled_pc]
                    df_cfg = pd.concat([df_cfg_other, pd.DataFrame(new_rows, columns=_CFG_COLS)], ignore_index=True)
                    save_smart_cfg(df_cfg)
                    st.success(f"✅ [{pc_sel_cat}] 설정 저장 완료! ({len(new_enabled_pc)}개 활성화)")
                    st.rerun()

        # ── Tab3: 유형별 원부자재 설정 ───────────────────────────────
        with tab_rm_cfg:
            st.markdown("#### 📦 원·부자재 유형별 적용 위해요소 설정")
            st.caption("원·부자재 등록의 유형(생두(원료), 포장재 등)을 선택하고, 해당 유형 전체에 스마트 적용할 위해요소를 설정합니다.")

            if _rm_list_sm.empty:
                st.warning("등록된 원·부자재가 없습니다.")
            else:
                # 유형 목록 추출 (등록 순서 유지, 중복 제거)
                _rm_type_col = "유형" if "유형" in _rm_list_sm.columns else None
                if _rm_type_col is None:
                    st.warning("원·부자재 등록에 '유형' 컬럼이 없습니다.")
                else:
                    _rm_types_all = [t for t in _rm_list_sm[_rm_type_col].str.strip().unique().tolist() if t]
                    if not _rm_types_all:
                        st.warning("유형이 설정된 원·부자재가 없습니다.")
                    else:
                        rm_type_sel = st.selectbox("원·부자재 유형 선택", _rm_types_all, key="smart_cfg_rm_type")
                        _safe_type_key = str(rm_type_sel).replace(" ","_").replace("(","").replace(")","")

                        mask_rm = (df_cfg["적용대상"] == "원부자재유형") & (df_cfg["대상키"] == rm_type_sel)
                        enabled_rm = set(zip(df_cfg[mask_rm]["유형"], df_cfg[mask_rm]["위해요소"]))

                        # 해당 유형에 속한 원부자재 코드 목록 표시
                        _rms_in_type = _rm_list_sm[_rm_list_sm[_rm_type_col].str.strip()==rm_type_sel]
                        _rm_names_in_type = ", ".join(_rms_in_type["원자재명"].tolist()) if not _rms_in_type.empty else "없음"
                        st.markdown(
                            f'<div style="background:#F0F4FF;border-left:3px solid #4A90D9;padding:7px 12px;border-radius:4px;margin:6px 0 10px;">'
                            f'📋 <b>[{rm_type_sel}]</b> 해당 원부자재: {_rm_names_in_type} &nbsp;|&nbsp; 현재 <b>{len(enabled_rm)}개</b> 항목 활성화</div>',
                            unsafe_allow_html=True
                        )

                        new_enabled_rm = set()
                        for hz_type in ["B", "C", "P"]:
                            type_label3 = {"B":"🦠 생물학적","C":"⚗️ 화학적","P":"🔩 물리적"}[hz_type]
                            df_ltr = df_sev_sm[df_sev_sm["유형"] == hz_type].reset_index(drop=True)
                            if df_ltr.empty: continue
                            st.markdown(f"**{type_label3}**")

                            all_checked_r = all((hz_type, r["위해요소"]) in enabled_rm for _, r in df_ltr.iterrows())
                            toggle_all_r = st.checkbox(f"전체 선택 ({type_label3})", value=all_checked_r, key=f"rm_all_{hz_type}_{_safe_type_key}")

                            for ri, row in df_ltr.iterrows():
                                key_r = (hz_type, row["위해요소"])
                                default_r = toggle_all_r or key_r in enabled_rm
                                checked_r = st.checkbox(
                                    f"{row['위해요소']}  [심각성 {row['심각성']}]",
                                    value=default_r,
                                    key=f"rm_chk_{hz_type}_{ri}_{_safe_type_key}"
                                )
                                if checked_r:
                                    new_enabled_rm.add(key_r)

                        if st.button("💾 설정 저장", type="primary", use_container_width=True, key="rm_cfg_save"):
                            df_cfg_other_r = df_cfg[~((df_cfg["적용대상"]=="원부자재유형") & (df_cfg["대상키"]==rm_type_sel))]
                            new_rows_r = [{"적용대상":"원부자재유형","대상키":rm_type_sel,"대상명":rm_type_sel,"유형":t,"위해요소":h}
                                          for (t,h) in new_enabled_rm]
                            df_cfg = pd.concat([df_cfg_other_r, pd.DataFrame(new_rows_r, columns=_CFG_COLS)], ignore_index=True)
                            save_smart_cfg(df_cfg)
                            st.success(f"✅ [{rm_type_sel}] 설정 저장 완료! ({len(new_enabled_rm)}개 활성화)")
                            st.rerun()

        # ── Tab4: CCP 이후 삭제 설정 ──────────────────────────────
        with tab_ccp_del:
            st.markdown("#### 🗑️ CCP 이후 자동 삭제 설정 (카테고리 · CCP 인스턴스별)")
            st.caption("카테고리를 선택하면 공정 흐름도에 등록된 CCP 단계(B1, B2, P1 등)별로 삭제 목록을 설정합니다.")

            df_ccp_del = load_ccp_del_cfg()

            if _cats_df_sm.empty or "cat_name" not in _cats_df_sm.columns:
                st.warning("공정 흐름도 카테고리가 없습니다. 공정 흐름도 메뉴에서 먼저 카테고리를 등록하세요.")
            else:
                del_cat_names = _cats_df_sm["cat_name"].tolist()
                del_cat_ids   = _cats_df_sm["cat_id"].tolist()
                del_sel_cat   = st.selectbox("카테고리 선택", del_cat_names, key="ccp_del_cat_sel")
                del_cat_idx   = del_cat_names.index(del_sel_cat)
                del_cat_id    = str(del_cat_ids[del_cat_idx])

                # 해당 카테고리 공정 흐름도에서 CCP유형 목록 추출
                _del_flow_file = f"haccp_flowchart_{del_cat_id}.csv"
                _del_ccp_types_in_flow = []
                if os.path.exists(_del_flow_file):
                    try:
                        _del_flow_df = pd.read_csv(_del_flow_file, dtype=str).fillna("")
                        if "CCP여부" in _del_flow_df.columns and "CCP유형" in _del_flow_df.columns:
                            _del_ccp_steps = _del_flow_df[
                                (_del_flow_df["CCP여부"].str.upper()=="Y") &
                                (_del_flow_df["CCP유형"].str.strip()!="")
                            ]
                            _del_ccp_types_in_flow = sorted(
                                _del_ccp_steps["CCP유형"].str.strip().unique().tolist()
                            )
                    except Exception: pass

                if not _del_ccp_types_in_flow:
                    st.info("공정 흐름도에 CCP 유형(B1, P1 등)이 설정된 단계가 없습니다.\n공정 흐름도 메뉴에서 CCP여부=Y 인 단계의 CCP 유형 컬럼에 B1, P1 등을 입력하세요.")
                else:
                    _del_cat_df = df_ccp_del[df_ccp_del["카테고리키"]==del_cat_id]
                    st.markdown(f"**현재 [{del_sel_cat}] 설정: {len(_del_cat_df)}개 항목 | 흐름도 CCP: {', '.join(_del_ccp_types_in_flow)}**")

                    # 기준 유형 색/라벨 매핑 (첫 글자 B/C/P 기준)
                    _BASE_COLOR = {"B":"#C0392B","C":"#D35400","P":"#7D3C98"}
                    _BASE_LABEL = {"B":"🦠 생물학적","C":"⚗️ 화학적","P":"🔩 물리적"}

                    for ccp_t_val in _del_ccp_types_in_flow:
                        base_t = ccp_t_val[0].upper() if ccp_t_val else ""
                        ccp_color = _BASE_COLOR.get(base_t, "#555555")
                        base_label = _BASE_LABEL.get(base_t, "")

                        st.markdown(
                            f'<div style="background:{ccp_color};color:#fff;font-weight:bold;padding:7px 14px;border-radius:6px;margin:16px 0 4px;">'
                            f'⚠️ {ccp_t_val} &nbsp;<span style="font-weight:normal;font-size:0.9em;">({base_label})</span> CCP 이후 삭제 목록</div>',
                            unsafe_allow_html=True
                        )
                        st.caption(f"[{del_sel_cat}] 흐름도의 {ccp_t_val} CCP 이후 공정에서 자동 삭제될 위해요소를 선택하세요.")

                        # 해당 기준 유형(B/C/P)의 심각성 목록
                        df_sev_t = df_sev_sm[df_sev_sm["유형"]==base_t].reset_index(drop=True)
                        if df_sev_t.empty:
                            st.caption(f"심각성 설정에 {base_t} 유형 위해요소가 없습니다.")
                            continue

                        enabled_del = set(
                            df_ccp_del[
                                (df_ccp_del["카테고리키"]==del_cat_id) &
                                (df_ccp_del["CCP유형"]==ccp_t_val)
                            ]["위해요소"].str.strip().tolist()
                        )

                        new_del_set = set()
                        _safe_key = ccp_t_val.replace(",","_")
                        all_del_checked = all(r["위해요소"].strip() in enabled_del for _, r in df_sev_t.iterrows())
                        toggle_all_del = st.checkbox(
                            f"전체 선택 ({ccp_t_val})", value=all_del_checked,
                            key=f"del_all_{_safe_key}_{del_cat_id}"
                        )
                        for dri, drow in df_sev_t.iterrows():
                            hz_nm_d = drow["위해요소"].strip()
                            default_d = toggle_all_del or hz_nm_d in enabled_del
                            chk_d = st.checkbox(
                                f"{hz_nm_d}  [심각성 {drow['심각성']}]",
                                value=default_d,
                                key=f"del_chk_{_safe_key}_{dri}_{del_cat_id}"
                            )
                            if chk_d:
                                new_del_set.add(hz_nm_d)

                        if st.button(f"💾 {ccp_t_val} 삭제 설정 저장", type="primary",
                                     use_container_width=True, key=f"del_save_{_safe_key}_{del_cat_id}"):
                            df_ccp_del_other = df_ccp_del[
                                ~((df_ccp_del["카테고리키"]==del_cat_id) & (df_ccp_del["CCP유형"]==ccp_t_val))
                            ]
                            new_del_rows = pd.DataFrame([
                                {"카테고리키":del_cat_id,"카테고리명":del_sel_cat,
                                 "CCP유형":ccp_t_val,"위해요소":h}
                                for h in new_del_set
                            ])
                            df_ccp_del = pd.concat([df_ccp_del_other, new_del_rows], ignore_index=True)
                            df_ccp_del.to_csv(CCP_DEL_FILE, index=False, encoding="utf-8-sig")
                            st.success(f"✅ [{del_sel_cat}] {ccp_t_val} 삭제 설정 저장 완료! ({len(new_del_set)}개)")
                            st.rerun()
